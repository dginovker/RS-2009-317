...
import org.gielinor.utilities.bzip2.BZip2Decompressor;
import org.gielinor.utilities.gzip.GZipDecompressor;
...
public final class ContainersInformation {
     ...
     */
    private CacheContainer informationContainer;
    ...
    public ContainersInformation(byte[] informationContainerPackedData) {
        ...
        informationContainer = new CacheContainer();
     ...
     */
    public CacheContainer getInformationContainer() {
    ...
    public void decodeContainersInformation(byte[] data) {
        ...
        for (int index = 0; index < containersIndexes.length; index++) {
            ...
            containers[containersIndexes[index]].setFiles(new CacheContainer[lastFileIndex + 1]);
            for (int fileIndex = 0; fileIndex < containers[containersIndexes[index]].getFilesIndexes().length; fileIndex++) {
                containers[containersIndexes[index]].getFiles()[containers[containersIndexes[index]].getFilesIndexes()[fileIndex]] = new CacheContainer();
            }
        }
        if (whirpool) {
            for (int index = 0; index < containersIndexes.length; index++) {
    ...
    /**
     * If is whirpool.
     * @return If is whirpool <code>True</code>.
     */
