...
import org.gielinor.gui.ConsoleTab;
...
public final class UtilityTab extends ConsoleTab {
    ...
    private static final org.slf4j.Logger log = LoggerFactory.getLogger(UtilityTab.class);
    ...
    public UtilityTab() {
        ...
        panel_1.setBorder(new TitledBorder(new EtchedBorder(EtchedBorder.LOWERED, null, null), "Wealth & Items",
        ...
        panel_2.setBorder(new TitledBorder(new EtchedBorder(EtchedBorder.LOWERED, null, null), "Player Utils",
            TitledBorder.LEADING, TitledBorder.TOP, null, null));
        ...
        panel_3.setBorder(new TitledBorder(new EtchedBorder(EtchedBorder.LOWERED, null, null), "Security",
            TitledBorder.LEADING, TitledBorder.TOP, null, null));
    ...
    public void removeMac() {
        ...
        // "!");
        // } else {
        // JOptionPane.showMessageDialog(null, "Error! Couldn't find mac in
        // system manager.");
        // }
    }
    ...
    public void viewMacs() {
        ...
        panel.add(pane);
        JOptionPane.showConfirmDialog(null, panel, "Banned Addresses", JOptionPane.OK_CANCEL_OPTION,
    ...
    public void wealthScanner() {
        ...
        if (args != null && args.length() > 0) {
            try {
                ...
                id = -1;
                log.warn("Bad int-string in wealth scanner?", ex);
            }
        }
        ...
        pane.setViewportView(list);
        panel.add(pane);
        JOptionPane.showConfirmDialog(null, panel, "Wealth Scanner", JOptionPane.OK_CANCEL_OPTION,
            JOptionPane.PLAIN_MESSAGE);
    ...
    public List<String> wealthScanner(int itemId) {
        ...
        for (Player p : players) {
            if (p.getDetails().getRights() == Rights.GIELINOR_MODERATOR) {
                continue;
            }
            if (item != null) {
                long old = count;
                count += p.getInventory().getCount(item);
                count += p.getEquipment().getCount(item);
                count += p.getBank().getCount(item);
                if (old != count) {
                    player++;
                }
            }
        ...
        for (int i = mostWealthy.length - 1; i >= 0; i--) {
            ...
            if (info != null && info[0] != null) {
                formats.add(i + ": " + info[1] + " - "
                    + NumberFormat.getNumberInstance(Locale.US).format((Long) info[0]) + " coins.");
            }
        }
        if (item != null) {
            formats.add(NumberFormat.getNumberInstance(Locale.US).format(count) + " occurrences of item "
                + item.getName() + " (id=" + item.getId() + ") by " + player + " players!");
        }
        return formats;
    }
    ...
    public void itemScanner() {
        ...
        pane.setViewportView(list);
        panel.add(pane);
        JOptionPane.showConfirmDialog(null, panel, "Item Scanner", JOptionPane.OK_CANCEL_OPTION,
            JOptionPane.PLAIN_MESSAGE);
    }
    ...
    public List<String> itemScanner(Item item) {
        ...
        for (Entry<String, Integer> entry : map.entrySet()) {
            formats.add("Player name=" + entry.getKey() + ", contained a total of " + entry.getValue() + " "
                + item.getName() + ".");
        }
        if (formats.size() == 0) {
    ...
    public void populateList() {
        ...
        TaskExecutor.execute(() -> {
            ...
            for (String name : ConsoleFrame.getInstance().getPlayerTab().getPlayerNames()) {
                ...
                if (details != null) {
                    addAddressToMap(name, details.getIp(), IPS);
    ...
    public void staffScanner() {
        ...
        for (Player player : players) {
            if (player.getDetails().getRights() != Rights.REGULAR_PLAYER) {
                staff.add(player.getName() + " - "
                    + TextUtils.formatDisplayName(player.getDetails().getRights().toString()));
            }
        }
        JPanel panel = new JPanel(new GridLayout(0, 1));
        JScrollPane pane = new JScrollPane();
        ...
        pane.setViewportView(list);
        panel.add(pane);
        JOptionPane.showConfirmDialog(null, panel, "Staff Scanner", JOptionPane.OK_CANCEL_OPTION,
            JOptionPane.PLAIN_MESSAGE);
    }
    ...
    public void delete(PlayerDetails details) {
        if (details.getRights() == Rights.GIELINOR_MODERATOR) {
            return;
        }
        // TODO
        ...
        // try {
        // statement = connection.prepareStatement("DELETE FROM " + "playerdata"
        // + " WHERE " + "" + "username" + "='" +
        // details.getUsername().toLowerCase() + "'");
        // statement.execute();
        // statement = connection.prepareStatement("DELETE FROM " + "highscores"
        // + " WHERE " + "" + "username" + "='" +
        // details.getUsername().toLowerCase() + "'");
        // statement.execute();
        ...
        // }
        //
        // File detailFile = new File(GameConstants.getPlayerDetailsPath() + "/"
        // + details.getUsername() + ".store");
        // File playerFile = new File(GameConstants.getPlayerSavePath() + "/" +
        // details.getUsername() + ".store");
        // detailFile.delete();
        // playerFile.delete();
