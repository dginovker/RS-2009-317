...
public final class Skillcape {
     ...
     */
    private static final int PURCHASE_COST = 99_000;
    ...
    public static boolean purchase(final Player player, final int skill) {
        ...
        if (!player.getInventory().containsItem(new Item(Item.COINS, PURCHASE_COST))) {
        ...
        if (player.getInventory().remove(new Item(Item.COINS, PURCHASE_COST))) {
    ...
    public static void trim(final Player player) {
        ...
        for (Container container : containers) {
            for (Item item : container.toArray()) {
                ...
                if (skill != -1) {
                    container.replace(getTrimmed(skill), item.getSlot());
                    skill = -1;
                }
            }
        }
