...
/**
 * Represents the egg and tomato recipe. This recipe consists of mixing a tomato with a scrambled egg.
 * @author 'Vexia
 * @date 22/12/2013
 */
public class EggAndTomato extends Recipe {
    ...
    public void mix(final Player player, final NodeUsageEvent event) {
        if (player.getSkills().getLevel(Skills.COOKING) < 23) {
            player.getDialogueInterpreter().sendPlaneMessage("You need a Cooking level of at least " + 23 + " in order to do this.");
            return;
        }
