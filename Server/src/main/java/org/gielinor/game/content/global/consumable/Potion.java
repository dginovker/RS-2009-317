...
public class Potion extends Drink {
    ...
    public void consume(final Item item, final Player player) {
        if (DuelRule.NO_DRINKS.enforce(player, true)) {
            return;
        }
    ...
    public void message(final Player player, final Item item, final int initial, String... messages) {
        ...
        player.getActionSender().sendMessage(item.getName().toLowerCase().contains("mix") ? "You drink the lumpy potion." : ("You drink some of " + (item.getName().contains("brew") ? "the foul liquid" : "your " + item.getName().replace("(4)", "").replace("(3)", "").replace("(2)", "").replace("(1)", "").trim().toLowerCase()) + "."), 1);
    }

    @Override
    public void healAndRemove(Player player, Item item) {
    ...
    public void remove(final Player player, final Item item, final int doses, boolean message) {
        ...
        int d = (4 - dose);
        if (item.getName().toLowerCase().contains("mix(")) {
            d = (2 - dose);
        }
        int replaceId = (int) (dose == 1 ? VIAL.getId() : effect.getItemIds()[d + 1]);
        if (replaceId != -1) {
            dose--;
            ...
            if (message) {
                ...
                player.getActionSender().sendMessage(m, 1);
            }
        }
    }
    ...
    public void effect(final Player player, final Item item) {
        ...
        if (b != null) {
            ...
            // Holy wrench / max cape
            if (MaxCapePlugin.isWearing(player) || MaxCapePlugin.containsCape(player)) {
                level += (player.getSkills().getStaticLevel(b.getSkillId()) > 25 &&
                    player.getSkills().getStaticLevel(b.getSkillId()) < 86) ? 1 :
                    (player.getSkills().getStaticLevel(b.getSkillId()) > 85) ? 2 : 0;
            }
