...
public abstract class EmoteClueScroll extends ClueScrollPlugin {
    ...
    /**
     * The {@link plugin.interaction.inter.EmoteTabInterface.Emote} to play.
     */
    private final Emote emote;

    /**
     * The commencing {@link plugin.interaction.inter.EmoteTabInterface.Emote}.
     */
    ...
    /**
     * The clue scroll interface text ids for text lines.
     */
    ...
    public boolean actionButton(Player player, int interfaceId, int buttonId, int slot, int itemId, int opcode) {
        ...
        if (this.getName().equalsIgnoreCase("ardgoune-mill-clap") && player.getLocation().getZ() != 2) {
            return false;
        }
        final Emote emote = Emote.forButtonId(buttonId);
        if (emote == null) {
            return false;
        }
        if (emote == this.emote) {
    ...
    public void read(Player player) {
        ...
        super.read(player);
        player.getActionSender().sendString(clue.replace("<br>", "<br><br>"), 6968); // TODO 317
    }
