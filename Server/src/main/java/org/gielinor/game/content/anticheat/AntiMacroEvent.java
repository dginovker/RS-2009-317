...
public abstract class AntiMacroEvent extends ActivityPlugin implements SavingModule {
    ...
    public static final Location[] LOCATIONS = new Location[]{
        Location.create(3218, 9616, 0),//Lumby basement
     ...
     * @param saveRequired Whether or not a save is required.
     * @param skillIds     The skill ids of the skills that fire this event (nothing for default).
     */
    ...
    public void noteItems() {
        ...
        for (Item item : player.getInventory().toArray()) {
            ...
            if (item.getDefinition().isUnnoted()) {
                ...
                player.getInventory().add(new Item(noteId, item.getCount()));
            }
        }
    ...
    public static Location getRandomLocation() {
        return LOCATIONS[RandomUtil.random(LOCATIONS.length)];
    }
