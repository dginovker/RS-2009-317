...
/**
 * Represents the fried mushroom recipe. This recipe consists of using a mushroom on a bowl.
 * @author 'Vexia
 * @date 22/12/2013
 */
public final class SlicedMushroom extends ToppingRecipe {
    ...
    public void mix(final Player player, final NodeUsageEvent event) {
        if (!player.getInventory().containsItem(KNIFE)) {
            player.getDialogueInterpreter().sendPlaneMessage("You need a knife in order to slice up the mushrooms.");
            return;
        }
