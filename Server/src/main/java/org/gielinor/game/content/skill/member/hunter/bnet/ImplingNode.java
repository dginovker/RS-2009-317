...
public final class ImplingNode extends BNetNode {
    ...
    public void loot(final Player player, final Item item) {
        ...
        final Item reward = RandomUtil.getChanceItem(getLoot()).getRandomItem();
        if (player.getInventory().remove(item)) {
            if (isBroken(player)) {
                player.getActionSender().sendMessage("You break the jar as you try and open it. You throw the shattered remains away.");
            } else {
    ...
    private boolean isBroken(Player player) {
        ...
        strengthLevel /= 0.5;
        int level = getLevel();
        int currentLevel = RandomUtil.random(strengthLevel) + 1;
        double ratio = currentLevel / (RandomUtil.random(level + 5) + 1);
    ...
    public void message(Player player, int type, boolean success) {
        ...
        switch (type) {
            case 1:
                player.getActionSender().sendMessage("You manage to catch the impling and squeeze it into a jar.");
                break;
        }
    }
