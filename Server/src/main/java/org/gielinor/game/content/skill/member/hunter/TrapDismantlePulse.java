...
public final class TrapDismantlePulse extends SkillPulse<GameObject> {
    ...
    public boolean checkRequirements() {
        ...
        if (player.getInventory().freeSlots() < itemCount) {
            player.getActionSender().sendMessage("You don't have enough inventory space. You need " + difference + " more free slot" + (difference > 1 ? "s" : "") + ".");
            return false;
        }
        return true;
    }
    ...
    public boolean reward() {
        ...
        if (wrapper.getType().getSettings().clear(wrapper, 1)) {
            ...
            if (wrapper.isCaught()) {
                player.getSkills().addExperience(Skills.HUNTER, wrapper.getReward().getExperience());
            }
            player.getActionSender().sendMessage("You dismantle the trap.");
        }
        return true;
    }
