...
/**
 * Represents the chopped tuna recipe. This recipe consists of adding tuna to a bowl with a knife.
 * @author 'Vexia
 * @date 22/12/2013
 */
public class ChoppedTuna extends ToppingRecipe {
    ...
    public void mix(final Player player, final NodeUsageEvent event) {
        if (!player.getInventory().containsItem(KNIFE)) {
            player.getDialogueInterpreter().sendPlaneMessage("You need a knife in order to slice up the tuna.");
            return;
        }
