...
public final class DeadfallSetting extends TrapSetting {
    ...
    public DeadfallSetting() {
        super(new int[]{ 28935, 19205 }, new Item[]{ new Item(946) }, new int[]{ 28937, 19206 }, new int[]{ 10138, 6006, 12574, 341, 2132 }, "set-trap", 23, -1, new Animation(5208), new Animation(5207), true);
    }

    @Override
    public boolean hasItems(Player player) {
        if (!super.hasItems(player)) {
            player.getActionSender().sendMessage("You need a knife in order to set a deadfall trap.");
            return false;
        }
        ...
        player.getActionSender().sendMessage("You need logs in order to set a deadfall trap.");
        return false;
    }

    @Override
    public TrapHook createHook(TrapWrapper wrapper) {
        List<Location> locations = getLocations(wrapper.getObject());
        return new TrapHook(wrapper, locations.toArray(new Location[locations.size()]));
    }
