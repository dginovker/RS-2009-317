package org.gielinor.game.content.skill.free.cooking.recipe.topping.impl;
...
/**
 * Represents the chilli con carne recipe. This recipe consists of using spicy sauce with cooked meat.
 * @author 'Vexia
 * @date 22/12/2013
 */
public class ChilliConCarne extends Recipe {
    ...
    public void mix(final Player player, final NodeUsageEvent event) {
        if (player.getSkills().getLevel(Skills.COOKING) < 9) {
            player.getDialogueInterpreter().sendPlaneMessage("You need a Cooking level of at least " + 9 + " in order to do this.");
            return;
        }
        if (!player.getInventory().containsItem(KNIFE)) {
            player.getDialogueInterpreter().sendPlaneMessage("You need a knife in order to cut up the meat.");
            return;
        }
        super.mix(player, event);
        player.getSkills().addExperience(Skills.COOKING, 25);
    }
