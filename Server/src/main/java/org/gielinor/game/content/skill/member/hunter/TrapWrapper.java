...
public final class TrapWrapper {
     ...
     *
     * @return <code>True</code> to deregister the trap.
     */
    public boolean cycle() {
        if (isTimeUp() && type.getSettings().clear(this, 0)) {
            if (!isCaught()) {
                player.getActionSender().sendMessage(type.getSettings().getTimeUpMessage());
            }
            return true;
        }
        return false;
    ...
    public void smoke() {
        if (smoked) {
            player.getActionSender().sendMessage("This trap has already been smoked.");
            return;
        }
        if (player.getHunterManager().getStaticLevel() < 39) {
            player.getActionSender().sendMessage("You need a Hunter level of at least 39 to be able to smoke traps.");
            return;
        }
        smoked = true;
        player.lock(4);
        player.visualize(new Animation(5208), new Graphics(931));
        player.getActionSender().sendMessage("You use the smoke from the torch to remove your scent from the trap.");
    }
    ...
    public void bait(Item bait) {
        if (baited) {
            player.getActionSender().sendMessage("This trap has already been baited.");
            return;
        }
        if (!type.getSettings().hasBait(bait)) {
            player.getActionSender().sendMessage("You can't use that on this trap.");
            return;
        }
        baited = true;
