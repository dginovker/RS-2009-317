...
public enum Talisman {
    ...
    public final void locate(final Player player) {
        if (this == ELEMENTAL || getRuin() == null) {
            player.getActionSender().sendMessage("You cannot tell which direction the Talisman is pulling...");
            return;
        }
        String direction = "";
        ...
        if (player.getLocation().getY() > loc.getY() && player.getLocation().getX() - 1 > loc.getX())
            ...
            direction = "west";
        player.getActionSender().sendMessage("The talisman pulls towards the " + direction + ".");
    }
