package org.gielinor.game.content.skill.member.fletching.items.grapple;
...
public final class GrapplePulse extends SkillPulse<Item> {
    ...
    public boolean checkRequirements() {
        int inventoryAmount = player.getInventory().getCount(GRAPPLE_TIP);
        ...
        if (player.getSkills().getLevel(Skills.FLETCHING) < 59) {
            player.getDialogueInterpreter().sendPlaneMessage("You need a fletching level of at least 59 in order to do this.");
            return false;
        }
        return true;
    }
