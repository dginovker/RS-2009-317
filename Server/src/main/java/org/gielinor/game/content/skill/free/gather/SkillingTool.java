...
public enum SkillingTool {
    ...
    /**
     * Represents an inferno adze (woodcutting & mining).
     */
     ...
     */
    DRAGON_PICKAXE(13264, 61, 0.82D, new Animation(10007));
    ...
    public static SkillingTool getHatchet(Player player) {
        ...
        } else if (checkInfernoAdze(player, Skills.WOODCUTTING, SkillingTool.INFERNO_ADZE)) {
    ...
    public static SkillingTool getPickaxe(Player player) {
        SkillingTool tool = null;
        if (checkTool(player, Skills.MINING, SkillingTool.DRAGON_PICKAXE)) {
        ...
        } else if (checkInfernoAdze(player, Skills.MINING, SkillingTool.INFERNO_ADZE)) {
            tool = SkillingTool.INFERNO_ADZE;
        }
        return tool;
    }
    ...
    public static boolean checkInfernoAdze(Player player, int skillId, SkillingTool tool) {
        if (player.getSkills().getStaticLevel(skillId) < (skillId == Skills.MINING ? 41 : 61)
            || player.getSkills().getStaticLevel(Skills.FIREMAKING) < 92) {
            return false;
        }
        if (player.getEquipment().getNew(3).getId() == tool.getId()) {
            return true;
