package org.gielinor.game.content.skill.free.cooking.recipe.pizza.impl;
...
/**
 * Represents the plain pizza recipe. This recipe consists of mixing tomato, and cheese to a pizza base.
 * @author 'Vexia
 * @date 22/12/2013
 */
public class PlainPizza extends Recipe {
    ...
    public void mix(final Player player, final NodeUsageEvent event) {
        if (player.getSkills().getLevel(Skills.COOKING) < 35) {
            player.getDialogueInterpreter().sendPlaneMessage("You need a Cooking level of at least " + 35 + " in order to do this.");
            return;
        }
