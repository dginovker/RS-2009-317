...
public final class GemBoltCutPulse extends SkillPulse<Item> {
    ...
    public boolean checkRequirements() {
        if (player.getSkills().getLevel(Skills.FLETCHING) < gem.getLevel()) {
            player.getDialogueInterpreter().sendPlaneMessage("You need a Fletching level of " + gem.getLevel() + " or above to do that.");
            return false;
        }
        return player.getInventory().containsItem(gem.getGem());
    }
    ...
    public boolean reward() {
        ...
        if (player.getInventory().remove(gem.getGem())) {
            player.getInventory().add(gem.getBoltTip());
            player.getSkills().addExperience(Skills.FLETCHING, 1);
        }
        amount--;
        return amount <= 0;
    }
