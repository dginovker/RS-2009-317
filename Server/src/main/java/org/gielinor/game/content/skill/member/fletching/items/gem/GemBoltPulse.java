...
public final class GemBoltPulse extends SkillPulse<Item> {
    ...
    public boolean checkRequirements() {
        if (player.getSkills().getLevel(Skills.FLETCHING) < bolt.getLevel()) {
            player.getDialogueInterpreter().sendPlaneMessage("You need a Fletching level of " + bolt.getLevel() + " or above to do that.");
            return false;
        }
        return !(!player.getInventory().containsItem(bolt.getBase()) || !player.getInventory().containsItem(bolt.getTip()));
    }
    ...
    public boolean reward() {
        ...
        if (player.getInventory().remove(bolt.getBase(), bolt.getTip())) {
            ...
            player.getSkills().addExperience(Skills.FLETCHING, bolt.getExperience());
