...
public final class PlantPotPulse extends ToolAction {
    ...
    public boolean canInteract(String command) {
        if (!player.getInventory().containsItem(FarmingConstant.GARDENING_TROWEL)) {
            player.getActionSender().sendMessage("You need a gardening trowel to do that.");
            return false;
        }
        if (wrapper.isWeedy()) {
            player.getActionSender().sendMessage("You need to weed this patch first.");
            return false;
        }
        if (!wrapper.isEmpty()) {
            player.getActionSender().sendMessage("You can only fill a plantpot from an empty farming patch.");
            return false;
        }
