...
/**
 * Represents the glass used to open the guide prices for the different type of trade(herbs, logs, runes, etc...)
 * @author 'Vexia
 * @date 30/11/2013
 */
public final class GEGuidePrice {
    ...
    public static final void clear(final Player player) {
        for (int i = 135; i < 165; i++) {
            //player.getActionSender().sendInterfaceConfig(COMPONENT.getId(), i, true);
        }
    }
    ...
    public enum GuideType {
        ...
        public void display(final Player player) {
            ...
            //player.getActionSender().sendString("Guide Prices: " + TextUtils.formatDisplayName(name()), COMPONENT.getId(), 14);
            for (int i = getChildData()[0]; i < getChildData()[1]; i++) {
                //player.getActionSender().sendInterfaceConfig(642, i, false);
            }
            for (GuideItem item : getItems()) {
                //	player.getActionSender().sendString("" + GrandExchangeDatabase.getDatabase().get(item.getItem()).getValue() + " gp", COMPONENT.getId(), item.getChildData()[0]);
            }
        }
    }
