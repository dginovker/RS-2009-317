...
public final class TrapCreatePulse extends SkillPulse<Node> {
    ...
    public boolean checkRequirements() {
        if (player.getHunterManager().getStaticLevel() < trap.getSettings().getLevel()) {
            player.getActionSender().sendMessage("You need a Hunter level of at least " + trap.getSettings().getLevel() + " in order to setup a " + node.getName().toLowerCase() + ".");
            return false;
        }
        if (player.getHunterManager().exceedsTrapLimit(trap)) {
            player.getActionSender().sendMessage(trap.getSettings().getLimitMessage(player));
            return false;
        }
        if (RegionManager.getObject(player.getLocation()) != null) {
            player.getActionSender().sendMessage("You can't lay a trap here.");
            return false;
        }
    ...
    public boolean reward() {
        if ((trap.getSettings().getSetupAnimation().getDefinition().getDurationTicks()) != 0) {
    ...
    public void message(int type) {
        switch (type) {
            case 0:
                setUp();
                player.getActionSender().sendMessage("You begin setting up the trap.");
                break;
        }
    }
