...
public final class CanoeExtension {
    ...
    /**
     * Represents the stage the player is at with crafting a canoe.(1=fallen, 2=crafted,3=floating)
     */
    private int stage;
     ...
     */
    private final Component shape = new Component(18178);

    /**
     * Represents the destination component.
     */
    private final Component destination = new Component(18220);
    ...
    /**
     * Method used to get the extension of this class, if its already there we return
     * it if not we create a new instance which is added to the player.
     *
     * @param player the player.
    ...
    public final void chop(final GameObject object) {
        ...
        if (player.getSkills().getLevel(Skills.WOODCUTTING) < 12) {
            player.getActionSender().sendMessage("You need a woodcutting level of at least 12 to chop down this tree.");
            return;
        }
        player.lock(3);
        ...
        World.submit(new Pulse(4, player) {
            ...
            public boolean pulse() {
                ...
                player.getActionSender().sendObjectAnimation(object, FALL, false);
                setStation(station);
                setStage(1);
                return true;
            }
    ...
    public final void shape(final GameObject object) {
        player.getInterfaceState().open(shape);
        for (int i = 0; i < Canoe.values().length; i++) {
            if (player.getSkills().getLevel(Skills.WOODCUTTING) < Canoe.values()[i].getLevel()) {
                continue;
            }
            player.getActionSender().sendHideComponent(52, shapeConfigs[i][0], true);
            player.getActionSender().sendHideComponent(52, shapeConfigs[i][1], false);
        }
    }
    ...
    public final void craft(final Canoe canoe) {
        if (player.getSkills().getLevel(Skills.WOODCUTTING) < canoe.getLevel()) {
            player.getActionSender().sendMessage("You need a woodcutting level of at least " + canoe.getLevel() + " to make this canoe.");
            return;
        }
        ...
        player.getPulseManager().run(new Pulse(3) {
            ...
            @Override
            public boolean pulse() {
                if (RandomUtil.random(canoe == Canoe.WAKA ? 8 : 6) == 1) {
    ...
    public final void floatCanoe(final GameObject object) {
        ...
        World.submit(new Pulse(1) {
            ...
            public boolean pulse() {
                if (counter == 1) {
                    player.getActionSender().sendObjectAnimation(object, FLOAT, false);
    ...
    public final void travel(final CanoeStation station) {
        ...
        if (player.getFamiliarManager().hasFamiliar()) {
            player.getActionSender().sendMessage("You can't take a follower on a canoe.");
            return;
        }
        player.lock(18);
        World.submit(new Pulse(1) {
            ...
            public boolean pulse() {
                switch (count++) {
                    ...
                    case 17:
                        ...
                        player.getActionSender().sendMessage("You arrive " + (station != CanoeStation.WILDERNESS ? "at " + station.getName() + "." : "in the wilderness. There are no trees suitable to make a canoe."));
                        player.getActionSender().sendMessage(station != CanoeStation.WILDERNESS ? "Your canoe sinks into the water after the hard journey." : "Your canoe sinks into the water after the hard journey. Looks like you're");
                        if (station == CanoeStation.WILDERNESS) {
                            player.getActionSender().sendMessage("walking back.");
                            player.getInterfaceState().openOverlay(new Component(380));
                        }
                        setCanoe(null);
                        setStation(null);
                        setStage(0);
    ...
    public final void paddle(final GameObject object) {
        player.getInterfaceState().open(destination);
        int index = station.ordinal();
        player.getActionSender().sendHideComponent(53, boatChilds[index], true);
        player.getActionSender().sendHideComponent(53, locationChilds[index], false);
        int distance = distances[canoe.ordinal()];
        for (int i = 0; i < 5; i++) {
            if (Math.abs(station.ordinal() - i) > distance) {
                player.getActionSender().sendHideComponent(53, boatChilds[i], true);
            }
        }
        if (canoe != Canoe.WAKA) {
            player.getActionSender().sendHideComponent(53, 31, true);
        }
    }
     ...
     *
     * @param tool The tool.
     * @return <code>True</code> if the tool is usable.
     */
