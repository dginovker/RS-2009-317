...
import org.gielinor.game.content.skill.member.farming.patch.Flowers;
import org.gielinor.game.content.skill.member.farming.patch.PatchProtection;
...
public class FarmingNode {
     ...
     *
     * @param cycle the cycle.
     * @return <code>True</code> if growing.
     */
    ...
    public boolean canDisease(final PatchCycle cycle) {
        if (cycle.getPlayer() != null) {
            // TODO diseases prevention for donators
			/*if (cycle.getPlayer().getDonorManager().isExtremeDonor()) {
				return false;
			}*/
			/*if (cycle.getPlayer().getDonorManager().isSuperDonor()) {
				if (!(cycle.getNode() instanceof TreeNode)) {
					return false;
				}
			}*/
        }
        ...
        int chance = 350;
        final int random = RandomUtil.random(chance);
        int mod = cycle.getWaterHandler().isWatered() ? 0 : (cycle.getCompostThreshold() > 1 ? 10 : RandomUtil.random(30, 50));
    ...
    public int getHarvestAmount(final PatchCycle cycle) {
        ...
        return min + RandomUtil.randomize((int) difference);
    }
