...
public class CookingProperties {
     ...
     */
    private static final Audio AUDIO = new Audio(65, 10, 0);
     ...
     */
    public static final String FAIL_MESSAGE = "Oops! You accidentally burnt the @name..";
    ...
    /**
     * Represents if its a range only food (meaning only can be used on a range not fire).
     */
    ...
    /**
     * Method used to cook a food. This method can be overried to intercept the way a food is cooked.
     * A prime example of how this can be used is with the sinew consumable food.
     *
    ...
    /**
     * Method used to cook a food. This method can be override to intercept the way a food is cooked.
     * A prime example of how this can be used is with the sinew consumable food.
     *
     * @param food   the food we're cooking.
     * @param player the player.
     * @param object the object.
     * @param burned if it's burned.
     * @return <code>True</code> if we we're successful.
     */
    public boolean cook(final Food food, final Player player, final GameObject object, final boolean burned) {
        return cook(food, player, object, burned, true);
    }
     ...
     * @return <code>True</code> if we we're successful.
     */
    public boolean cook(final Food food, final Player player, final GameObject object, final boolean burned, final boolean sendMessage) {
        if (player.getInventory().remove(food.getRaw())) {
            player.getInventory().add(burned ? food.getBurnt() : food.getItem());
            player.getSkills().addExperience(Skills.COOKING, burned ? 0 : getExperience());
            if (sendMessage) {
                player.getActionSender().sendMessage(getMessage(food, player, object, burned), 1);
            }
            player.getActionSender().sendSound(AUDIO);
            if (!burned && object.getLocation().equals(Location.create(3212, 3215, 0))) {
                AchievementDiary.finalize(player, AchievementTask.BAKE_BREAD);
            }
            return true;
        }
        return false;
    ...
    public boolean cookAll(Player player, GameObject node) {
        boolean isTrue = false;
        int randomNumber = RandomUtil.random(101);
        if (randomNumber <= player.getDonorManager().getDonorStatus().getCookAllFoodChance()
            && player.getDonorManager().getDonorStatus().getCookAllFoodChance() != -1) {
            for (Item item : player.getInventory().getItems()) {
                Food food1 = Consumables.forRaw(item.getId());
                if (food1 != null) {
                    cook(food1, player, node, false, false);
                    isTrue = true;
                }
            }
            player.getActionSender().sendMessage("<col=FF0000>All the raw food in your inventory all become cooked too!</col>");
    ...
    /**
     * Gets the burning level depicted by if the player has cooking gauntlets on, or effected
     * by the object they are interactign with.
     *
    ...
    public boolean isBurned(final Player player, final GameObject object) {
        ...
        if (MaxCapePlugin.isWearing(player)) {
            return false;
        }
        double burn_chance = (55.0 - (object.getName().equals("fire") ? 0.0 : 3.0));
