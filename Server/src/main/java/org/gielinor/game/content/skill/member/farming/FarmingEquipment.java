...
public final class FarmingEquipment implements SavingModule {
     ...
     */
    public static final Item[] ITEMS = new Item[]{ new Item(5341), new Item(5343), new Item(952), new Item(5329), new Item(5331), new Item(5325), new Item(1925), new Item(6032), new Item(6034) };
    ...
    public void open(final Player player) {
        ...
        player.getInterfaceState().open(STORE_COMPONENT);
        player.getInterfaceState().openSingleTab(TAB_COMPONENT);
    ...
    public boolean store(final Player player, final int slot, int amount) {
        ...
        if (!hasItem(player, slot, false)) {
            player.getActionSender().sendMessage("You haven't got " + (name.equals("secateurs") || name.equals("supercompost") || name.equals("compost") ? "any " : "a ") + name + " to store.");
            return false;
        }
        final int inventoryAmount = player.getInventory().getCount(slot == 4 ? new Item(getWateringCan(player).getId(), 1) : ITEMS[slot]);
        ...
        if ((getEquipmentAmount(slot) + amount) > getMaxAdd(slot)) {
            player.getActionSender().sendMessage("You cannot store more than " + getAddName(slot) + " " + name + (getMaxAdd(slot) > 1 ? "s" : "") + " in here.");
            return false;
        }
        ...
        if (player.getInventory().remove(item)) {
            ...
            container.replace(new Item(item.getId(), amount), slot, true);
            update(player);
            return true;
        }
        return false;
    ...
    public boolean remove(final Player player, final int slot, int amount) {
        ...
        if (!hasItem(player, slot, true)) {
            player.getActionSender().sendMessage("You haven't got " + (name.equals("secateurs") || name.equals("supercompost") || name.equals("compost") ? "any " : "a ") + name + " stored in here.");
            return false;
        }
        ...
        if (!player.getInventory().hasRoomFor(item)) {
            player.getActionSender().sendMessage("You don't have room to hold that.");
            return false;
        }
    ...
    public int getInventoryAmount(final Player player, int slot) {
        if (slot == 4) {
            return player.getInventory().getCount(getWateringCan(player));
        }
        if (slot < 0) {
            return 0;
        }
        return player.getInventory().getCount(ITEMS[slot]);
    }
    ...
    public static final class StoreCloseEvent implements CloseEvent {
        ...
        public void close(Player player, Component c) {
            ...
            player.getActionSender().sendRunScript(101, "");
        }
