...
public final class CompostBin {
    ...
    public void fillBin(final Player player, final Item item, int delay) {
        final String[] messages = isRotted(player) ? new String[]{ "The compost bin must be empty of compost before you can put new", "items in it." } : isClosed(player) ? new String[]{ "The compost bin is closed" } : !container.hasRoomFor(item) ? new String[]{ "The compost bin is too full to put anything else in it." } : null;
        if (messages != null) {
    ...
    public void fillBucket(final Player player, int delay) {
        ...
        if (isTomato() && !player.getInventory().hasRoomFor(FarmingConstant.ROTTEN_TOMATO)) {
    ...
    public void close(final Player player) {
        final boolean test = player.isDebug();
        setTimeStamp(System.currentTimeMillis() + (test ? 0 : (TimeUnit.MINUTES.toMillis(isSuperCompostable() ?
            FarmingConstant.SUPERCOMPOST_TIME : FarmingConstant.COMPOST_TIME))));
        addConfigValue(player, 31 << getBitShift());
        player.animate(CLOSE_ANIM);
        player.getActionSender().sendMessage("You close the compost bin.");
        player.getActionSender().sendMessage("The " + (isTomato() ? "tomatoes" : "contents") + " have begun to rot.");
    }
    ...
    public void open(final Player player) {
        if (isRotting(player)) {
            player.getDialogueInterpreter().sendPlaneMessage(isTomato() ? "The tomatoes haven't finished rotting yet." : "The vegetation hasn't finished rotting yet.");
            return;
        }
        player.animate(OPEN_ANIM);
        addConfigValue(player, getCompostMultiplier(player) << getBitShift());
        player.getActionSender().sendMessage("You open the compost bin.");
    }
    ...
    public boolean isSuperCompost(final Player player) {
        return getState(player) == 48 || getState(player) == 62;
    }
    ...
    public boolean isTomato() {
        return !(container.getCount(FarmingConstant.TOMATO) == 0 || container.itemCount() == 0) &&
            container.getCount(FarmingConstant.TOMATO) == container.itemCount();
    }
    ...
    public int getState(final Player player) {
        ...
        if (def == null) {
            //			System.err.println("Config file definition was null!");
            return 0;
        }
    ...
    public final class FillBinPulse extends Pulse {
        ...
        private int getWeedDifference() {
            int amount = player.getInventory().getCount(FarmingConstant.WEEDS);
