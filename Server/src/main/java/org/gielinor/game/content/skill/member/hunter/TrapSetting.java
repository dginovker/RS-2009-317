...
public class TrapSetting {
    ...
    public void investigate(Player player, GameObject object) {
        ...
        player.getActionSender().sendMessage("This trap " + (wrapper.isSmoked() ? "has" : "hasn't") + " been smoked.");
    }
    ...
    public void catchNpc(final TrapWrapper wrapper, final TrapNode node, final NPC npc) {
        ...
        World.submit(getCatchPulse(wrapper, node, npc, success));
    }
    ...
    public boolean isSuccess(Player player, final TrapNode node) {
        ...
        int roll = RandomUtil.random(99 - 30);//(player.getDetails().getShop().hasPerk(Perks.PROWLER) ? 34 : 10));
