package org.gielinor.game.content.skill.member.fletching.items.crossbow;
...
public class LimbPulse extends SkillPulse<Item> {
    ...
    public boolean checkRequirements() {
        if (player.getSkills().getLevel(Skills.FLETCHING) < limb.getLevel()) {
            player.getDialogueInterpreter().sendPlaneMessage("You need a fletching level of " + limb.getLevel() + " to attach these limbs.");
            return false;
        }
    ...
    public boolean reward() {
        ...
        if (player.getInventory().remove(limb.getStock(), limb.getLimb())) {
            ...
            player.getSkills().addExperience(Skills.FLETCHING, limb.getExperience());
            player.getActionSender().sendMessage("You attach the metal limbs to the stock.");
        }
        if (!player.getInventory().containsItem(limb.getLimb()) || !player.getInventory().containsItem(limb.getStock())) {
            return true;
        }
        amount--;
        return amount == 0;
