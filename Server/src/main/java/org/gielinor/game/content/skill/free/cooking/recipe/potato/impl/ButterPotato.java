package org.gielinor.game.content.skill.free.cooking.recipe.potato.impl;
...
/**
 * Represents the butter potato recipe. This recipe consists of adding a pat of butter to a potato.
 * @author 'Vexia
 * @date 22/12/2013
 */
public class ButterPotato extends Recipe {
    ...
    public void mix(final Player player, final NodeUsageEvent event) {
        if (player.getSkills().getLevel(Skills.COOKING) < 39) {
            player.getDialogueInterpreter().sendPlaneMessage("You need a Cooking level of at least " + 39 + " in order to do this.");
            return;
        }
        super.singleMix(player, event);
        player.getSkills().addExperience(Skills.COOKING, 95.5);
    }
