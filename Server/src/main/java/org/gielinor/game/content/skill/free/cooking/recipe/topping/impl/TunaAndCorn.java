...
/**
 * Represents the tuna and corn recipe. This recipe consists of adding cooked sweetcorn to a bowl of chopped tuna.
 * @author 'Vexia
 * @date 22/12/2013
 */
public final class TunaAndCorn extends Recipe {
    ...
    public void mix(final Player player, final NodeUsageEvent event) {
        if (player.getSkills().getLevel(Skills.COOKING) < 67) {
            player.getDialogueInterpreter().sendPlaneMessage("You need a Cooking level of at least " + 57 + " in order to do this.");
            return;
        }
        super.mix(player, event);
        player.getSkills().addExperience(Skills.COOKING, 204);
    }
