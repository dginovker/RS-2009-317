...
public final class GrowthHandler {
     ...
     *
     * @return <code>True</code> if we should stop.
     */
    public boolean grow() {
    ...
    public long getGrowthUpdate() {
        ...
        return (weedy ? TimeUnit.MINUTES.toMillis(5) : TimeUnit.MINUTES.toMillis(cycle.getNode().getMinutes() / 2));
    }

    /**
     * Checks if its a weedy growth.
     *
     * @return <code>True</code> if do.
     */
    ...
    public boolean canGrow() {
        return cycle.getGrowthTime() < System.currentTimeMillis() && cycle.getGrowthTime() != 0L
            && (isWeedy() || isGrowing() || cycle.getDiseaseHandler().isDiseased() || cycle.getWaterHandler().isWatered());
    }
    ...
    public boolean isGrowing() {
        return cycle.getNode() != null && cycle.getNode().isGrowing(cycle);
    }
