...
public final class Appearance implements SavingModule {
    ...
    /**
     * Represents the item id to represent if not -1.
     */
    private int itemId = -1;

    /**
     * Represents the object id to represent if not -1.
     */
    ...
    public void save(ByteBuffer buffer) {
        ...
        for (BodyPart anAppearanceCache : appearanceCache) {
            anAppearanceCache.save(buffer);
        }
    }
    ...
    public void parse(ByteBuffer buffer) {
        ...
        for (BodyPart anAppearanceCache : appearanceCache) {
            anAppearanceCache.parse(buffer);
        }
    }

    /**
     * Transforms the player into an item.
     *
     * @param id The item id.
     */
    public void transformItem(int id) {
        this.npcId = -1;
        this.itemId = id;
        this.objectId = -1;
        sync();
    ...
    /**
     * Transforms the player into an object.
     *
     * @param id The object id.
     */
    public void transformObject(int id) {
        this.npcId = -1;
        this.itemId = -1;
        this.objectId = id;
        setAnimations();
        if (id == -1) {
            ...
            player.getProperties().setDeathAnimation(new Animation(/*9055*/836, Priority.HIGH));
        } else {
            ObjectDefinition objectDefinition = ObjectDefinition.forId(id);
            player.setSize(objectDefinition == null ? 1 : objectDefinition.getSizeX());
            if (objectDefinition != null) {
                setStandAnimation(objectDefinition.animationId);
                setStandTurnAnimation(objectDefinition.animationId);
                setWalkAnimation(objectDefinition.animationId);
                setRunAnimation(objectDefinition.animationId);
                setTurn180(objectDefinition.animationId);
                setTurn90cw(objectDefinition.animationId);
                setTurn90ccw(objectDefinition.animationId);
            }
        }
        sync();
    }
    ...
    public void transformNPC(int id) {
        ...
        if (id == -1) {
            ...
            player.setSize(def == null ? 1 : def.getSize());
            if (def != null) {
                ...
                if (def.getCombatAnimations()[0] != null) {
                    ...
                    player.playAnimation(player.getProperties().getAttackAnimation());
                }
    ...
    public void copy(Appearance appearance) {
        ...
        System.arraycopy(appearance.appearanceCache, 0, appearanceCache, 0, appearanceCache.length);
        System.arraycopy(appearance.animationCache, 0, animationCache, 0, animationCache.length);
    }
    ...
    public void prepareBodyData(Player player) {
        ...
        boolean castleWarsHood = cape != null && (cape.getId() == 4041 || cape.getId() == 4042); //Item ids for the hoods?
        if (hat != null) {
            drawItem(0, hat);
        } else {
            clearBodyPart(0);
    ...
    /**
     * Gets the value <code>True</code> if male.
     *
     ...
     *
     * @return If the player's appearance is a NPC <code>True</code>.
     */
    ...
    /**
     * If the player's appearance is an item.
     *
     * @return If the player's appearance is an item <code>true</code>.
     */
    ...
    /**
     * If the player's appearance is an object.
     *
     * @return If the player's appearance is an object <code>True</code>.
     */
    ...
    public static class AppearanceCache {
        ...
        /**
         * Represents the index hair appearance is cached at.
         */
