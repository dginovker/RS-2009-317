...
public abstract class IdleAbstractNPC extends AbstractNPC {
    ...
    public boolean isAttackable(Entity entity, CombatStyle style) {
        ...
        if (getTask() != null && entity instanceof Player && getTask().getLevel() > entity.getSkills().getStaticLevel(Skills.SLAYER)) {
            ((Player) entity).getActionSender().sendMessage("You need a higher slayer level to know how to wound this monster.");
        }
        if (DeathTask.isDead(this)) {
            return false;
        }
        if (!entity.getZoneMonitor().continueAttack(this, style, false)) {
            return false;
        }
        return true;
    }
