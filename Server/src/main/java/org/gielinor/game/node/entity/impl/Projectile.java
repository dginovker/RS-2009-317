...
import org.gielinor.game.world.update.flag.chunk.ProjectileUpdateFlag;
...
public class Projectile {
     ...
     * @param destination  The targeted destination.
     * @param projectileId The projectile's gfx id;
     * @param startHeight  The starting height.
     * @param endHeight    The ending height.
     * @param startDelay   The startDelay.
    ...
    public static Projectile magic(Location source, Location victim, int projectileId, int startHeight, int endHeight, int startDelay, int angle) {
        int speed = (int) (46 + (source.getDistance(victim.getLocation()) * 10));
        return new Projectile(source, victim, projectileId, startHeight, endHeight, startDelay, speed, angle, source.size() << 5);
    }
    ...
    public static Projectile ranged(Location source, Location victim, int projectileId, int startHeight, int endHeight, int startDelay, int angle) {
        int speed = (int) (46 + (source.getDistance(victim.getLocation()) * 5));
        return new Projectile(source, victim, projectileId, startHeight, endHeight, startDelay, speed, angle, source.size() << 5);
    }
     ...
     * @param distance     The distance.
     */
    private Projectile(Location start, Location location, int projectileId, int startHeight, int endHeight, int startDelay, int speed, int angle, int distance) {
     ...
     */
    public static void send(Projectile projectile) {
        RegionManager.getRegionChunk(projectile.getSourceLocation()).flag(new ProjectileUpdateFlag(projectile));
    }

    /**
     * Transforms the projectile (to start from the source & end at the victim) and sends it.
     *
     * @param source The source.
     * @param victim The victim.
    ...
    /**
     * Transforms the projectile (to start from the source & end at the locations) and sends it.
     *
     * @param source The source.
     * @param l      The location.
     * @param p      The projectile to send.
    ...
    /**
     * @param endLocation the endLocation1 to set
     */
    ...
    public String toString() {
        return Projectile.class.getName() + " source=" + source +
