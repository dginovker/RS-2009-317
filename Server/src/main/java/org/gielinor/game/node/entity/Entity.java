package org.gielinor.game.node.entity;
...
import org.gielinor.game.node.entity.impl.Properties;
import org.gielinor.game.node.entity.impl.PulseManager;
import org.gielinor.game.node.entity.impl.WalkingQueue;
import org.gielinor.game.node.entity.lock.ActionLocks;
...
import org.gielinor.game.node.entity.state.StateManager;
import org.gielinor.game.world.World;
import org.gielinor.game.world.map.Location;
import org.gielinor.game.world.map.Viewport;
import org.gielinor.game.world.map.path.Path;
import org.gielinor.game.world.map.path.Pathfinder;
import org.gielinor.game.world.map.zone.ZoneMonitor;
import org.gielinor.game.world.update.UpdateMasks;
...
public abstract class Entity extends Node {
     ...
     * The teleporter.
     */
    private final Teleport teleporter = new Teleport(this);
    ...
    /**
     * Resets the entity's update flags, ...
     * <br>Gets called after {@link #update()}.
     */
    public void reset() {
        updateMasks.reset();
    ...
    public void finalizeDeath(Entity killer) {
        ...
        face(null);
        if (this instanceof NPC && killer instanceof Player) {
            if (((Player) killer).getEquipment().contains(1321)) {
                AchievementDiary.finalize(((Player) killer), AchievementTask.NO_TIME_WASTED);
            }
        }
    }
    ...
    public void attack(final Node node) {
        if ((node instanceof Player && this instanceof Player) && !Ironman.canAttack((Player) this, (Player) node)) {
            return;
        }
    ...
    public boolean canMove(Location destination) {
        return true;//RegionManager.isClipped(destination);
    }
    ...
    /**
     * Gets the projectile lockon index of this entity.
     *
     * @return The projectile lockon index of this entity.
     */
    public abstract int getProjectileLockonIndex();

    /**
     * Checks if the npc should ignore victim attack restrictions(i.e You can't attack this npc.")
     *
     * @param victim the victim.
     ...
     * @return <code>True</code> if the attacking entity can attack this entity.
     */
    public boolean isAttackable(Entity entity, CombatStyle style) {
        if (DeathTask.isDead(this)) {
            return false;
        }
        if (!entity.getZoneMonitor().continueAttack(this, style, true)) {
            return false;
        }
        if (this instanceof Player && entity instanceof Player) {
            return !Ironman.isIronman((Player) this) && !Ironman.isIronman((Player) entity);
        }
        return true;
    }
     ...
     */
    public void playGraphics(Graphics graphic) {
        graphics(graphic);
    }
    ...
    public boolean graphics(final Graphics graphics, int delay) {
        World.submit(new Pulse(delay, this) {
    ...
    public boolean canSelectTarget(Entity target) {
        if (target instanceof Player) {
            return ((Player) target).getSavedData().getGlobalData().getVisibility() == 0;
        }
        return true;
    }
    ...
    public boolean faceTemporary(Entity entity, final Entity reset, int ticks) {
        if (face(entity)) {
            World.submit(new Pulse(ticks + 1, this) {
     ...
     *
     * @return <code>True</code> if the entity is immune to poison.
     */
    ...
    public void saveAttribute(String key, Object value) {
        attributes.saveAttribute(key, value);
    }
    ...
    public void setTeleportTarget(Location teleportTarget) {
        ...
        this.getProperties().setTeleportLocation(teleportTarget);
    }
    ...
    public void teleport(final Location location, int ticks) {
        World.submit(new Pulse(ticks, this) {
