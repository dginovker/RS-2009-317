...
public enum SpellType {
    ...
    BOLT(1.1) {
        @Override
        public int getImpactAmount(Entity e, Entity victim, int base) {
            if (e instanceof Player && ((Player) e).getEquipment().getNew(Equipment.SLOT_HANDS).getId() == 777) {
                return 11 + base;
            }
            return 8 + base;
        }
    ...
    GOD_STRIKE(1.2) {
        ...
        public int getImpactAmount(Entity e, Entity victim, int base) {
            if (e.getStateManager().hasState(EntityState.CHARGED)) {
                Item cape = ((Player) e).getEquipment().getNew(Equipment.SLOT_CAPE);
                if (cape.getId() == 2412 || cape.getId() == 2413 || cape.getId() == 2414 || cape.getId() == 11874 || cape.getId() == 11876 || cape.getId() == 11878) {
                    return 30;
                }
            }
            return 20;
