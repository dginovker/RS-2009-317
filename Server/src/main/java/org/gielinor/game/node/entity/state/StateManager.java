...
public final class StateManager implements SavingModule {
    ...
    public void save(ByteBuffer buffer) {
        for (EntityState state : states.keySet()) {
            ...
            if (pulse == null) {
                log.warn("Pulse for state [{}] is null.", state);
                continue;
            }
    ...
    public void parse(ByteBuffer buffer) {
        ...
        try {
            ...
            log.error("Failed to parse state.", ex);
        }
    }
    ...
    /**
     * Puts a pulse for the given state.
     *
     * @param state The state to put the pulse for.
     */
    public void put(EntityState state, StatePulse statePulse) {
        states.put(state, statePulse);
    }
