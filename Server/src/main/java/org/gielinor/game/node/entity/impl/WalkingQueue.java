...
public final class WalkingQueue {
    ...
    private double getEnergyRestore(Player player) {
        ...
        if (EquipmentSet.GRACEFUL_INVENTORY.isUsing(player) || EquipmentSet.GRACEFUL.isUsing(player)) {
    ...
    public void updateRunEnergy(boolean decrease) {
        ...
        if (p.getDonorManager().getDonorStatus().getInfiniteRunEnergy()) {
            decrease = false;
        }
     ...
     *
     * @return <code>True</code> if the player is teleporting, <code>False</code> if not.
     */
     ...
     *
     * @return <code>True</code> if the region updated, <code>False</code> if not.
     */
     ...
     *
     * @return <code>True</code> if a ctrl + click action was performed,
     * <br>		the player has the run option enabled or the NPC is a familiar,
     * <p>		<code>False</code> if not.
     */
    public boolean isRunningBoth() {
        return entity instanceof Player && ((Player) entity).getSettings().isRunToggled() || running;
    }
