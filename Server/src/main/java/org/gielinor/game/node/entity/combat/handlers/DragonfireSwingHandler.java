...
public class DragonfireSwingHandler extends CombatSwingHandler {
    ...
    public void visualizeImpact(Entity entity, Entity victim, BattleState state) {
        if (entity instanceof NPC && victim instanceof Player) {
            ...
            p.getActionSender().sendMessage(getDragonfireMessage(victim.getAttribute("fire_resistance", 0), !fire ? "icy breath" : "fiery breath"));
            Item shield = p.getEquipment().get(Equipment.SLOT_SHIELD);
            if (shield != null && (shield.getId() == 11283 || shield.getId() == 11284)) {
                if (shield.getId() == 11284) {
                    p.getEquipment().replace(new Item(11283), Equipment.SLOT_SHIELD);
                    shield = p.getEquipment().get(Equipment.SLOT_SHIELD);
                    shield.setCharge(0);
                }
                if (shield.getCharge() < 1000) {
                    shield.setCharge(shield.getCharge() + 20);
                    Equipment.updateBonuses(p);
                    p.getActionSender().sendMessage("Your dragonfire shield glows more brightly.");
                } else {
                    p.getActionSender().sendMessage("Your dragonfire shield is already fully charged.");
                }
        ...
        if (!fire && victim.getAttribute("freeze_immunity", -1) < World.getTicks() && RandomUtil.random(4) == 2) {
