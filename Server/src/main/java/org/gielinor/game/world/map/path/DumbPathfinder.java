...
/**
 * A pathfinder implementation used for an easy path, where the pathfinder won't find a way around clipped objects..
 * <br>This is used for NPC combat following, NPC random movement, etc.
 * @author Emperor
 *
 */
    ...
    public Path find(Location start, int size, Location end, int sizeX, int sizeY, int rotation, int type, int walkingFlag, boolean near) {
        ...
        while (x != end.getX() || y != end.getY()) {
            ...
            if (sizeX != 0 && sizeY != 0) {
                ...
                if (directions.length > 1) { //Ensures we approach the location correctly (non-diagonal).
            ...
            switch (dir) {
                ...
                case NORTH_EAST:
                    if ((RegionManager.getClippingFlag(z, x + 1, y) & 0x12c0180) != 0
                        || (RegionManager.getClippingFlag(z, x, y + 1) & 0x12c0120) != 0
                        || (RegionManager.getClippingFlag(z, x + 1, y + 1) & 0x12c01e0) != 0) {
                        found = false;
                        break;
                    }
                ...
                case SOUTH_EAST:
                    if ((RegionManager.getClippingFlag(z, x + 1, y) & 0x12c0180) != 0
                        || (RegionManager.getClippingFlag(z, x, y - 1) & 0x12c0102) != 0
                        || (RegionManager.getClippingFlag(z, x + 1, y - 1) & 0x12c0183) != 0) {
                        found = false;
                        break;
                    }
                    points.add(new Point(x + 1, y - 1, dir));
                ...
                case SOUTH_WEST:
                    if ((RegionManager.getClippingFlag(z, x - 1, y) & 0x12c0108) != 0
                        || (RegionManager.getClippingFlag(z, x, y - 1) & 0x12c0102) != 0
                        || (RegionManager.getClippingFlag(z, x - 1, y - 1) & 0x12c010e) != 0) {
                        found = false;
                        break;
                    }
                    points.add(new Point(x - 1, y - 1, dir));
                ...
                case NORTH_WEST:
                    if ((RegionManager.getClippingFlag(z, x - 1, y) & 0x12c0108) != 0
                        || (RegionManager.getClippingFlag(z, x, y + 1) & 0x12c0120) != 0
                        || (RegionManager.getClippingFlag(z, x - 1, y + 1) & 0x12c0138) != 0) {
                        found = false;
                        break;
                    }
                    points.add(new Point(x - 1, y + 1, dir));
        ...
        for (Direction dir : directions) {
            ...
            switch (dir) {
                case NORTH:
                    ...
                    break;
                case NORTH_EAST:
                    if ((RegionManager.getClippingFlag(z, x + 1, y + 2) & 0x12c0138) != 0
                        || (RegionManager.getClippingFlag(z, x + 2, y + 2) & 0x12c01e0) != 0
                        || (RegionManager.getClippingFlag(z, x + 2, y + 1) & 0x12c0183) != 0) {
                        found = false;
                        break;
                    }
                    points.add(new Point(x + 1, y + 1, dir));
                    ...
                    break;
                case EAST:
                    if ((RegionManager.getClippingFlag(z, x + 2, y) & 0x12c0183) != 0
                        || (RegionManager.getClippingFlag(z, x + 2, y + 1) & 0x12c01e0) != 0) {
                        found = false;
                        break;
                    }
                    points.add(new Point(x + 1, y, dir));
                    ...
                    break;
                case SOUTH_EAST:
                    if ((RegionManager.getClippingFlag(z, x + 1, y - 1) & 0x12c010e) != 0
                        || (RegionManager.getClippingFlag(z, x + 2, y) & 0x12c01e0) != 0
                        || (RegionManager.getClippingFlag(z, x + 2, y - 1) & 0x12c0183) != 0) {
                        found = false;
                        break;
                    }
                    ...
                    break;
                case SOUTH:
                    if ((RegionManager.getClippingFlag(z, x, y - 1) & 0x12c010e) != 0
                        || (RegionManager.getClippingFlag(z, x + 1, y - 1) & 0x12c0183) != 0) {
                        found = false;
                        break;
                    }
                ...
                case SOUTH_WEST:
                    if ((RegionManager.getClippingFlag(z, x - 1, y - 1) & 0x12c010e) != 0
                        || (RegionManager.getClippingFlag(z, x - 1, y) & 0x12c0138) != 0
                        || (RegionManager.getClippingFlag(z, x, y - 1) & 0x12c0183) != 0) {
                        found = false;
                        break;
                    }
                    points.add(new Point(x - 1, y - 1, dir));
        ...
        for (Direction dir : directions) {
            ...
            switch (dir) {
                case NORTH:
                    ...
                    break;
                case NORTH_EAST:
                    if ((RegionManager.getClippingFlag(z, x + 1, y + size) & 0x12c0138) != 0
                        || (RegionManager.getClippingFlag(z, x + size, y + size) & 0x12c01e0) != 0
                        || (RegionManager.getClippingFlag(z, x + size, y + 1) & 0x12c0183) != 0) {
                        found = false;
                        break;
                    }
                    for (int i = 1; i < size - 1; i++) {
                    ...
                    break;
                case EAST:
                    if ((RegionManager.getClippingFlag(z, x + size, y) & 0x12c0183) != 0
                        || (RegionManager.getClippingFlag(z, x + size, y + (size - 1)) & 0x12c01e0) != 0) {
                        found = false;
                        break;
                    }
                    for (int i = 1; i < size - 1; i++) {
                ...
                case SOUTH_EAST:
                    if ((RegionManager.getClippingFlag(z, x + 1, y - 1) & 0x12c010e) != 0
                        || (RegionManager.getClippingFlag(z, x + size, y + (size - 2)) & 0x12c01e0) != 0
                        || (RegionManager.getClippingFlag(z, x + size, y - 1) & 0x12c0183) != 0) {
                        found = false;
                        break;
                    }
                ...
                case SOUTH:
                    if ((RegionManager.getClippingFlag(z, x, y - 1) & 0x12c010e) != 0
                        || (RegionManager.getClippingFlag(z, x + (size - 1), y - 1) & 0x12c0183) != 0) {
                        found = false;
                        break;
                    }
                    for (int i = 1; i < size - 1; i++) {
                    ...
                    break;
                case SOUTH_WEST:
                    if ((RegionManager.getClippingFlag(z, x - 1, y + (size - 2)) & 0x12c0138) != 0
                        || (RegionManager.getClippingFlag(z, x - 1, y - 1) & 0x12c010e) != 0
                        || (RegionManager.getClippingFlag(z, x + (size - 2), y - 1) & 0x12c0183) != 0) {
                        found = false;
                        break;
                    }
                    for (int i = 1; i < size - 1; i++) {
