...
public final class DynamicRegion extends Region {
    ...
    public static DynamicRegion copy(int regionId, Location to) {
        ...
        DynamicRegion region = new DynamicRegion(regionId, (to.getRegionX() + 6) >> 3, (to.getRegionY() + 6) >> 3);
    ...
    public void replaceChunk(int z, int x, int y, RegionChunk chunk, Region r) {
        ...
        if (chunk == null) {
            ...
            RegionPlane regionPlane = r.getPlanes()[l.getZ()];
            for (int i = 0; i < 8; i++) {
                for (int j = 0; j < 8; j++) {
                    int toX = (x << 3) + i;
                    int toY = (y << 3) + j;
                    ...
                    p.getFlags().getLandscape()[toX][toY] = regionPlane.getFlags().getLandscape()[fromX][fromY];
                    p.getFlags().getClippingFlags()[toX][toY] = regionPlane.getFlags().getClippingFlags()[fromX][fromY];
                    p.getProjectileFlags().getClippingFlags()[toX][toY] = regionPlane.getProjectileFlags().getClippingFlags()[fromX][fromY];
                    GameObject object = regionPlane.getObjects()[fromX][fromY];
                    if (object != null) {
