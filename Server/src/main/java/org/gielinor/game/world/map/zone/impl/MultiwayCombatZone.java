...
import org.gielinor.net.packet.context.MultiCombatContext;
...
public final class MultiwayCombatZone extends MapZone {
    ...
    public void configure() {
        ...
        registerRegion(10554);
        // Ape atoll
        registerRegion(11051);
        // Zombies
        registerRegion(9622);
        // Corp lair
        register(new ZoneBorders(2882, 4371, 2917, 4408));
        // Jatizso
        registerRegion(9531);
    }
    ...
    public boolean enter(Entity e) {
        if (e instanceof Player) {
            PacketRepository.send(MultiCombatPacket.class, new MultiCombatContext((Player) e, true));
        }
    ...
    public boolean leave(Entity e, boolean logout) {
        if (!logout && e instanceof Player) {
            PacketRepository.send(MultiCombatPacket.class, new MultiCombatContext((Player) e, false));
        }
    ...
    public boolean move(Entity e, Location loc, Location destination) {
        ...
        if (!player) {
            ...
            if (dir.getStepX() != 0 && dir.getStepY() != 0) {
                return true; //Allow diagonal steps so people can still "stack" npcs (see barraging mummies)
            }
        }
