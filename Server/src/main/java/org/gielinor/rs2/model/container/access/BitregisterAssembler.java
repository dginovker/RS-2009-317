package org.gielinor.rs2.model.container.access;

import org.gielinor.game.node.entity.player.Player;

/**
 * Contains the mask value methods.
 * The 'access mask' is actually just a bit register that contains permissions for a specific interface (@Peterbjornx)
 * @date 5/02/2013
 * @author Stacx
 */
public final class BitregisterAssembler {
    ...
    /**
     * Holds the first 16 (32 total) values for our bitregister.
     * Each one holds a 'permission' for the interface. (ie. draggable, option)
     * Left out the rest 16 because they're mostly for other things than interface options.
     * {@link http://www.rune-server.org/runescape-development/rs-503-client-server/informative-threads/184175-about-optionmask-values.html}
     */
    ...
    public static void send(Player player, int interfaceIndex, int childIndex, int offset, int length, int count) {
        ...
        player.getActionSender().sendAccessMask(calculateRegister(count), childIndex, interfaceIndex, offset, length);
    }
