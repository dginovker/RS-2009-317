...
import org.gielinor.game.content.skill.free.crafting.gem.GemCutPulse;
    ...
    public boolean handle(NodeUsageEvent event) {
        ...
        SkillDialogueHandler handler = new SkillDialogueHandler(player, SkillDialogue.ONE_OPTION, gem.getGem()) {
            ...
            public int getAll(int index) {
                return player.getInventory().getCount(gem.getUncut());
            }

        };
        if (player.getInventory().getCount(gem.getUncut()) == 1) {
            handler.create(0, 1);
        } else {
            handler.open();
        }
