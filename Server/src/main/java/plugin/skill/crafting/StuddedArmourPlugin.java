...
public final class StuddedArmourPlugin extends UseWithHandler {
    ...
    public boolean handle(final NodeUsageEvent event) {
        ...
        SkillDialogueHandler handler = new SkillDialogueHandler(player, SkillDialogue.ONE_OPTION, armour.getStudded()) {
            ...
            public int getAll(int index) {
                return player.getInventory().getCount(STEEL_STUDS);
            }

        };
        if (player.getInventory().getCount(armour.getItem()) == 1) {
            handler.create(1, 0);
        } else {
            handler.open();
        }
    ...
    public enum StuddedArmour {
        CHAPS(new Item(1095), new Item(1097), 18, 27),
        BODY(new Item(1129), new Item(1133), 14, 25);
    ...
    public static final class StudArmourPulse extends SkillPulse<Item> {
        ...
        public boolean checkRequirements() {
            if (player.getSkills().getLevel(Skills.CRAFTING) < armour.getLevel()) {
                player.getActionSender().sendMessage("You need a Crafting level of at least " + armour.getLevel() + " to do this.");
                return false;
            }
            if (!player.getInventory().containsItem(STEEL_STUDS)) {
                player.getActionSender().sendMessage("You need studs in order to make studded armour.");
                return false;
            }
        ...
        public boolean reward() {
            ...
            if (player.getInventory().remove(armour.getItem(), STEEL_STUDS)) {
                ...
                player.getSkills().addExperience(Skills.CRAFTING, armour.getExperience());
                player.getActionSender().sendMessage("You make a " + armour.getStudded().getName().toLowerCase() + ".", 1);
            }
            amount--;
            return amount < 1;
        }
        ...
        public void message(int type) {
            switch (type) {
                case 0:
                    player.getActionSender().sendMessage("You use the studs with the " + node.getName().toLowerCase() + ".", 1);
                    break;
            }
        }
    }
