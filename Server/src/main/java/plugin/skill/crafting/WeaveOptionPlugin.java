    ...
    public static final class WeavePulse extends SkillPulse<GameObject> {
        ...
        public boolean checkRequirements() {
            if (player.getSkills().getLevel(Skills.CRAFTING) < type.getLevel()) {
                player.getActionSender().sendMessage("You need a Crafting level of at least " + type.getLevel() + " in order to do this.");
                return false;
            }
            if (!player.getInventory().containsItem(type.getRequired())) {
                player.getActionSender().sendMessage("You need " + type.getRequired().getCount() + " " + type.getRequired().getName().toLowerCase().replace("ball", "balls") + "" + (type == WeavingItem.SACK ? "s" : type == WeavingItem.CLOTH ? "" : "es") + " to weave " + (TextUtils.isPlusN(type.getProduct().getName().toLowerCase()) ? "an" : "a") + " " + type.getProduct().getName().toLowerCase() + ".");
                return false;
            }
            return true;
        }
        ...
        public boolean reward() {
            ...
            if (player.getInventory().remove(type.getRequired())) {
                ...
                player.getActionSender().sendMessage("You weave the " + type.getRequired().getName().toLowerCase().replace("ball", "balls") + "" + (type == WeavingItem.SACK ? "s" : type == WeavingItem.CLOTH ? "" : "es") + " into " + (TextUtils.isPlusN(type.getProduct().getName().toLowerCase()) ? "an" : "a") + " " + type.getProduct().getName().toLowerCase() + ".", 1);
            }
            amount--;
            return amount < 1;
        }
