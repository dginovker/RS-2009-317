                    ...
                    switch (optionSelect) {
                        ...
                        case FOUR_OPTION_TWO:
                            if (!player.getHouseManager().hasHouse()) {
                                player.getActionSender().sendMessage("You should talk to the estate agent to get a house first.");
                                break;
                            }
                            if (player.getHouseManager().getLocation().getPortalId() != player.getAttribute("con:portal", -1)) {
                                player.getActionSender().sendMessage("Your house is in " + player.getHouseManager().getLocation().name().toLowerCase() + ".");
                                break;
                            }
                            player.getHouseManager().enter(player, optionSelect == OptionSelect.FOUR_OPTION_TWO);
                            break;
                            ...
                            player.setAttribute("runscript", new RunScript() {
                                ...
                                public boolean handle() {
                                    ...
                                    if (!p.getHouseManager().isInHouse()) {
                                        player.getActionSender().sendMessage("They do not seem to be at home.");
                                        return true;
                                    }
                                    if (p.getHouseManager().getLocation() != location) {
                                        player.getActionSender().sendMessage("That friend's house is not located here.");
                                        return true;
                                    }
