...
public final class TelekineticGrabSpell extends MagicSpell {
     ...
     */
    private static final Audio AUDIO = new Audio(3007, 10, 1);
     ...
     */
    public static final int SPELL_ID = 1168;

    /**
     * Constructs a new {@code TelekineticGrabSpell} {@code Object}.
     */
    public TelekineticGrabSpell() {
        super(SpellBook.MODERN, 33, 43, ANIMATION, START_GRAPHIC, AUDIO, new Item[]{ new Item(Runes.AIR_RUNE.getId()), new Item(Runes.LAW_RUNE.getId(), 1) });
    }
    ...
    public boolean cast(final Entity entity, final Node target) {
        ...
        World.submit(getGrabPulse(entity, ground));
        return true;
    }
            ...
            public boolean pulse() {
                ...
                if (player != null) {
                    if (g == null || !g.isActive()) {
                        player.getActionSender().sendMessage("Too late!");
                        return true;
                    }
                    player.getAudioManager().send(3008);
                    player.getInventory().add(new Item(g.getId(), g.getCount(), g.getCharge()));
                    player.getActionSender().sendPositionedGraphics(END_GRAPHIC, ground.getLocation());
                }
    ...
    public boolean canCast(final Entity entity, final GroundItem item) {
        ...
        if (entity instanceof Player) {
            ...
            if (!player.getInventory().hasRoomFor(((Item) item))) {
