...
public final class CurePlantSpell extends MagicSpell {
    ...
    public Plugin<SpellType> newInstance(SpellType arg) throws Throwable {
        SpellBook.LUNAR.register(30025, this);
        return this;
    }
    ...
    public boolean cast(Entity entity, Node target) {
        ...
        if (wrapper == null) {
            player.getDialogueInterpreter().sendDialogues(player, FacialExpression.NO_EXPRESSION, "Umm... This spell won't cure that!");
            return false;
        }
        if (wrapper.getCycle().getGrowthHandler().isFullGrown()) {
            player.getActionSender().sendMessage("That's not diseased.");
            return false;
        }
        if (!wrapper.getCycle().getDiseaseHandler().isDiseased() &&
            (wrapper.getCycle().getGrowthHandler().isGrowing() || wrapper.getCycle().getWaterHandler().isWatered())) {
            player.getActionSender().sendMessage("It is growing just fine.");
            return false;
        }
        if (wrapper.isWeedy()) {
            player.getActionSender().sendMessage("The weeds are healthy enough already.");
            return false;
        }
        if (wrapper.isEmpty()) {
            player.getActionSender().sendMessage("There's nothing there to cure.");
            return false;
        }
        if (wrapper.getCycle().getDeathHandler().isDead()) {
            player.getActionSender().sendMessages("It says 'Cure' not 'Resurrect'. Although death may arise from disease,", "it is not in itself a disease and hence cannot be cured. So there.");
            return false;
        }
        ...
        player.getSkills().addExperience(Skills.FARMING, 91.5);
        return true;
    }
}
