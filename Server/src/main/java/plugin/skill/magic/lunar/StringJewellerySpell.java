...
public final class StringJewellerySpell extends MagicSpell {
    ...
    public Plugin<SpellType> newInstance(SpellType arg) throws Throwable {
        SpellBook.LUNAR.register(31086, this);
        return this;
    }
    ...
    public boolean cast(Entity entity, Node target) {
        ...
        if (!contains) {
            p.getActionSender().sendMessage("You need jewellery in order to use this spell.");
            return false;
        }
    ...
    public class LunarStringPulse extends SkillPulse<Item> {
        ...
        public boolean reward() {
            ...
            if (player.getInventory().remove(node) && meetsRequirements(player, true, true)) {
                player.getInventory().add(new Item(STRUNG[getIndex()]));
                player.getSkills().addExperience(Skills.CRAFTING, 4);
            } else {
                return true;
            }
