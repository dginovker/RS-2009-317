...
/**
 * <b>Turns one log into a plank, without need of the sawmill. Coins are still required to convert a log into a plank, but
 * the amount is decreased.</b>
 *
 * @author 'Vexia
 */
public class PlankMakeSpell extends MagicSpell {
    ...
    public Plugin<SpellType> newInstance(SpellType arg) throws Throwable {
        SpellBook.LUNAR.register(30242, this);
        return this;
    }
    ...
    public boolean cast(Entity entity, Node target) {
        ...
        if (plank == null) {
            player.getActionSender().sendMessage("You need to use this spell on logs.");
            return false;
        }
        if (!player.getInventory().contains(Item.COINS, plank.getPrice())) {
            player.getActionSender().sendMessage("You need " + plank.getPrice() + " coins to convert that log into a plank.");
            return false;
        }
        player.lock(4);
        create(player, plank, player.getInventory().getCount(item));
        return true;
    }
    ...
    public final void create(final Player player, final Plank plank, int amount) {
        ...
        Item coins = new Item(Item.COINS, plank.getPrice() * amount);
     ...
     */
    public enum Plank {
        WOOD(new Item(1511), new Item(960), 70),
        OAK(new Item(1521), new Item(8778), 175),
        TEAK(new Item(6333), new Item(8780), 350),
        MAHOGANY(new Item(6332), new Item(8782), 1050);
