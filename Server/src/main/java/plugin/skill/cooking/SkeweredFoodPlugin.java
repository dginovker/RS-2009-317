...
public class SkeweredFoodPlugin extends UseWithHandler {
    ...
    public boolean handle(NodeUsageEvent event) {
        ...
        if (player.getSkills().getLevel(Skills.FIREMAKING) < LEVEL) {
            player.getActionSender().sendMessage("You meed a Firemaking level of at least " + LEVEL + " in order to do this.");
            return true;
        }
    ...
    public enum SkeweredSet {
        RABBIT(new Item(3226), new Item(7224)),
        CHOMPY(new Item(2876), new Item(7230)),
        BIRD(new Item(9978), new Item(9984)),
        BEAST(new Item(9986), new Item(9992));
