...
/**
 * Represents a cooking recipe plugin. This is used to handle the multiple recipes used in cooking
 * these recipes can range from making a pizza or making a pie.
 * @author 'Vexia
 * @version 1.9
 */
                ...
                for (int k = 0; k < temp.getParts().length; k++) {
                    for (int i = 0; i < temp.getIngredients().length; i++) {
                        ...
                        if (part.getId() == event.getUsedItem().getId() && ingredient.getId() == event.getBaseItem().getId() || part.getId() == event.getBaseItem().getId() && ingredient.getId() == event.getUsedItem().getId()) {
                            if (k == i) {//represents that this ingredient can mix with the other.
                                recipe = temp;
                                break;
                            }
                        }
