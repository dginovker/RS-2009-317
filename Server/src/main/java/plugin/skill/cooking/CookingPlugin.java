...
/**
 * Represents the plugin used to handle the interaction of a cooking consumable with a range/fire.
 *
 * @author 'Vexia
 * @version 1.0
 */
public final class CookingPlugin extends UseWithHandler {
     ...
     */
    private static final int[] OBJECTS = new int[]{ 2728, 2729, 2730, 2731, 2732, 2859, 3038, 3039, 3769, 3775, 4265, 4266, 5249, 5499, 5631, 5632, 5981, 7181, 9682, 10433, 11404, 11405, 11406, 12102, 12796, 13337, 13881, 14169, 14919, 15156, 20000, 20001, 21620, 21792, 22713, 22714, 23046, 24283, 24284, 25155, 25156, 25465, 25730, 26181, 27297, 29139, 30017, 32099, 33500, 34495, 34546, 36973, 37597, 37629, 37726, 114, 4172, 5275, 8750, 16893, 22154, 34410, 34565, 114, 9085, 9086, 9087, 12269, 15398, 25440, 25441, 2724, 2725, 2726, 4618, 4650, 5165, 6093, 6094, 6095, 6096, 8712, 9439, 9440, 9441, 10824, 17640, 17641, 17642, 17643, 18039, 21795, 24285, 24329, 27251, 33498, 35449, 36815, 36816, 37426 };
    ...
    public boolean handle(NodeUsageEvent event) {
        ...
        if (object.getLocation().equals(Location.create(2856, 3334))) {
            if (event.getPlayer().sendDonorNotification("to use this range.")) {
                return true;
            }
        }
    ...
    public Location getDestination(final Player player, final Node node) {
        if (node.getName().toLowerCase().equals("fire")) {
            ...
            if (node.getLocation().equals(Location.create(2817, 3444, 0))) {
                return null;
            }
