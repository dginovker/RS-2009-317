...
import org.gielinor.game.content.skill.free.cooking.fermenting.WineFermentingPulse;
...
public final class WineFermentPlugin extends UseWithHandler {
    ...
    public boolean handle(NodeUsageEvent event) {
        ...
        if (player.getSkills().getLevel(Skills.COOKING) < 35) {
            player.getActionSender().sendMessage("You need a cooking level of 35 to do this.");
            return true;
        }
        if (player.getInventory().remove(GRAPES, JUG_OF_WATER)) {
            player.getInventory().add(UNFERMENTED_WINE);
            World.submit(new WineFermentingPulse(1, player));
        }
        return true;
    }
