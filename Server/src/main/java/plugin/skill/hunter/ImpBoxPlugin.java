...
public class ImpBoxPlugin extends OptionHandler {
    ...
    public static class ImpBoxDialogue extends DialoguePlugin {
        ...
        public boolean open(Object... args) {
            interpreter.sendDialogues(708, FacialExpression.ANGRY, MESSAGES[RandomUtil.getRandom(MESSAGES.length)]);
            return true;
        }
            ...
            if (item != null) {
                if (player.getBank().canAdd(item) && item.getId() != box.getId()) {
                    ...
                    if (box.getId() == IDS[1]) {
                        ...
                        player.getInterfaceState().close(component);
                        player.getActionSender().sendMessage(FINISHING_MESSAGE);
                    }
                }
            } else {
                player.getActionSender().sendMessage("You cannot add this item to your bank.");
                return false;
            }
            return true;
        }
