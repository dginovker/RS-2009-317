...
import org.gielinor.game.content.skill.member.hunter.TrapHook;
...
public final class HunterNPC extends AbstractNPC {
    ...
    protected Location getMovementDestination() {
        if (trap.getHooks().size() == 0 || RandomUtil.random(170) > 5) {
            return super.getMovementDestination();
        }
        TrapHook hook = trap.getHooks().get(RandomUtil.random(trap.getHooks().size()));
