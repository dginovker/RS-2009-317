...
import org.gielinor.game.content.skill.member.farming.pot.Saplings;
...
public final class FarmingPlugin extends OptionHandler {
    ...
    /**
     * Represents the plugin used to handle a farm tool interaction with a patch.
     *
     * @author 'Vexia
     * @version 1.0
     */
...
//			ComponentDefinition.put(126, this);
                ...
                case 125:
                    switch (button) {
                        case 68:
                            ...
                            break;
                        case 69:
                            final int amount = getAmount(player, slot + 6, opcode, true);
                            if (amount == -1) {
                ...
                case 126:
                    switch (button) {
                        case 0:
                            ...
                            break;
                        case 1:
                            final int amount = getAmount(player, slot + 6, opcode, false);
                            if (amount == -1) {
                                player.setAttribute("runscript", new RunScript() {
         ...
         * @return the amount.
         */
        private int getAmount(final Player player, int slot, int opcode, boolean withdraw) {
            if (opcode == 157) {
                if (withdraw) {
                    if (player.getFarmingManager().getEquipment().getContainer().get(slot) == null) {
                        return 0;
                    }
                    return player.getFarmingManager().getEquipment().getContainer().get(slot).getCount();
                }
                return player.getFarmingManager().getEquipment().getInventoryAmount(player, slot);
            }
            return opcode == 133 ? 1 : opcode == 236 ? 5 : -1;
        }
    }
            ...
            public boolean handle(NodeUsageEvent event) {
                ...
                if (first.getId() == HAY_SACK.getId() && second.getId() == BRONZE_SPEAR.getId()) {
                    if (player.getInventory().remove(first, second)) {
                        ...
                        player.getActionSender().sendMessage("You stick the bronze spear through the sack.");
                        return true;
                    }
                } else if (first.getId() == 6058 && second.getId() == WATERMELON.getId()) {
                    if (player.getInventory().remove(first, second)) {
                        player.getInventory().add(SCARECROW);
                        player.getSkills().addExperience(Skills.FARMING, 25);
                        player.getActionSender().sendMessage("You stick a watermelon on top of the hay sack.");
                        return true;
                    }
                }
                return false;
    ...
    public static final class PotSaplingPlugin extends UseWithHandler {
        ...
        public boolean handle(NodeUsageEvent event) {
            ...
            if (player.getSkills().getLevel(Skills.FARMING) < sapling.getLevel()) {
                player.getActionSender().sendMessage("You need a Farming level of at least " + sapling.getLevel() + " to do that.");
                return true;
            }
            if (player.getInventory().remove(sapling.getSeed(), event.getUsedItem())) {
                player.getInventory().add(sapling.getSeedling());
                player.getActionSender().sendMessage("You sow " + (TextUtils.isPlusN(sapling.getSeed().getName()) ? "an" : "a") + " " + sapling.getSeed().getName().toLowerCase() + " in the plant pot.", 1);
                player.getActionSender().sendMessage("It needs watering before it will grow.", 1);
            }
            return true;
        }
        ...
        public final static class PotWateringPlugin extends UseWithHandler {
            ...
            public boolean handle(NodeUsageEvent event) {
                ...
                if (seedling.getCharge() > 1000) {
                    player.getActionSender().sendMessage("The seedling has already been watered.");
                    return true;
                }
                ...
                player.getActionSender().sendMessage("You water the " + sapling.getSeed().getName().toLowerCase() + ".", 1);
                return true;
            }
