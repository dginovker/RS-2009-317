...
public final class FarmHolderPlugin implements Plugin<Object> {
    ...
    public static final class HaySackPlugin extends UseWithHandler {
        ...
        public boolean handle(NodeUsageEvent event) {
            ...
            if (player.getInventory().remove(event.getUsedItem())) {
                ...
                player.getActionSender().sendMessage("You fill the sack with straw.");
            }
            return true;
        }
    ...
    public static final class ItemHolderPlugin extends OptionHandler {
        ...
        public boolean handle(Player player, Node node, String option) {
            ...
            switch (option) {
                case "fill":
                    ...
                    if (base == null) {
                        player.getActionSender().sendMessage(sack ? "You don't have any potatoes, onions, or cabbages." : "You don't have any fruit to fill the basket with.");
                        return true;
                    }
                    final Item remove = new Item(base.getId(), player.getInventory().getCount(base));
                    ...
                    if (holder.getAmount(item) == defaultMax) {
                        player.getActionSender().sendMessage("The " + (sack ? "sack" : "fruit basket") + " is full.");
                        return true;
                    }
        ...
        private void remove(final Player player, final Item item, final FarmingItemHolder holder, final int amount) {
            ...
            if (!player.getInventory().hasRoomFor(remove)) {
                player.getActionSender().sendMessage("You don't have enough inventory space.");
                return;
            }
            if (player.getInventory().add(remove)) {
                player.getInventory().replace(holder.getItem(holder.getAmount(item) - remove.getCount()), item.getSlot());
                player.getActionSender().sendMessage("You take " + (remove.getCount() < 2 ? (TextUtils.isPlusN(remove.getName().toLowerCase()) ? "an" : "a") : "" + remove.getCount()) + " " + remove.getName().toLowerCase() + (remove.getCount() > 1 ? "s" : "") + " out of the " + (holder.ordinal() < 3 ? "sack" : "basket") + ".");
            }
        }
