...
public final class BrimhavenArena extends MapZone implements Plugin<Object> {
     ...
     */
    private static final Component OVERLAY = new Component(8287);
    ...
    private void setDispenser(Player player) {
        ...
        int index = RandomUtil.randomize(DISPENSERS.length);
        ...
        player.setAttribute("brim-icon",
            HintIconManager.registerHeightHintIcon(player, DISPENSERS[index], 50));
    }
        ...
        if (node instanceof GameObject) {
            ...
            if (object.getId() == 3608 || object.getId() == 3581) {
                ...
                if (!object.getLocation().equals(DISPENSERS[player.getAttribute("brim-tag", 0)])) {
                    player.getActionSender().sendMessage("Tag the pillar indicated by the yellow arrow to get an Agility ticket.");
                    return true;
                }
                if (player.getAttribute("brim-tagged", false)) {
                    player.getActionSender().sendMessage("You have already tagged this pillar.");
                    return true;
                }
                ...
                if (tags > 1) {
                    ...
                    player.getActionSender().sendMessage("You have received an Agility Arena Ticket!");
                    return true;
                }
                player.getActionSender().sendMessage("You get tickets by tagging more than one pillar in a row, tag the next pillar!");
                player.getDialogueInterpreter().sendPlaneMessage("You get tickets by tagging more than one pillar in a row. <col=C04000>Tag the", "<col=C04000>next pillar for a ticket!</col>");
                return true;
            }
        }
        return false;
    ...
    private static void handlePlankSwitching() {
        ...
        for (int i = 0; i < 1 + RandomUtil.randomize(2); i++) {
            available[RandomUtil.randomize(3)] = true;
        }
        for (int i = 0; i < 3; i++) {
    ...
    private static enum PlankSet {
        FIRST(new Location[]{ Location.create(2797, 9591, 3), Location.create(2797, 9590, 3), Location.create(2797, 9589, 3) },
            new Location[]{ Location.create(2802, 9591, 3), Location.create(2802, 9590, 3), Location.create(2802, 9589, 3) }),
        SECOND(new Location[]{ Location.create(2764, 9558, 3), Location.create(2764, 9557, 3), Location.create(2764, 9556, 3) },
            new Location[]{ Location.create(2769, 9558, 3), Location.create(2769, 9557, 3), Location.create(2769, 9556, 3) });
