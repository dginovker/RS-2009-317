...
public final class BrimhavenCourse extends OptionHandler {
        ...
        switch (object.getId()) {
            case 3566:
                ...
                if (AgilityHandler.hasFailed(player, 1, 0.1)) {
                    AgilityHandler.failWalk(player, 2, start, start.transform(-dir.getStepX(), -dir.getStepY(), 0), player.getLocation().transform(0, 0, -3),
                        Animation.create(1105), 26, getHitAmount(player), "You missed the rope!").setEndAnimation(Animation.RESET);
                    return true;
                }
                AgilityHandler.forceWalk(player, -1, start, end, Animation.create(751), 25, 20.0, null);
                World.submit(new Pulse(1, player) {
                    ...
                    public boolean pulse() {
                        if (!finish) {
                            player.getActionSender().sendObjectAnimation(object, Animation.create(1052));
                            finish = true;
                            return false;
                        }
                        player.getActionSender().sendObjectAnimation(object, Animation.create(-1));
                        return true;
                    }
                });
                return true;
            ...
            case 3565:
                ...
                AgilityHandler.forceWalk(player, -1, start, start.transform(dir.getStepX() * 3, dir.getStepY() * 3, 0), Animation.create(1252), 6, 8.0, null);
                return true;
            case 3553:
            case 3557:
    ...
    private static void handleHandHolds(final Player player, final GameObject object) {
        if (player.getSkills().getLevel(Skills.AGILITY) < 20) {
            player.getActionSender().sendMessage("You need an agility of at least 20 to get past this obstacle!");
            return;
        }
        int mod = 0;
        ...
        World.submit(new Pulse(3, player) {
            ...
            public boolean pulse() {
                if (++count == 1) {
                    ...
                    player.getAppearance().setAnimations();
                    player.getAppearance().sync();
                    AgilityHandler.forceWalk(player, -1, last, last.transform(dir), Animation.create(1120 + m), 5, 22.0, null).setDirection(faceDirection);
                    return true;
                }
    ...
    private static void handleBalancingRope(final Player player, GameObject object) {
        ...
        AgilityHandler.walk(player, -1, player.getLocation(), player.getLocation().transform(dir.getStepX() * 7, dir.getStepY() * 7, 0), Animation.create(762), 10.0, null);
    }
    ...
    private static void handlePlankObstacle(final Player player, GameObject object) {
        ...
        AgilityHandler.walk(player, -1, start, end, ForceMovement.WALK_ANIMATION, 6.0, null);
    }
    ...
    private static void handlePillarObstacle(final Player player, GameObject object) {
        ...
        World.submit(new Pulse(2, player) {
            ...
            public boolean pulse() {
                ...
                AgilityHandler.forceWalk(player, -1, player.getLocation(), player.getLocation().transform(dir), Animation.create(741), 10, count == 5 ? 18.0 : 0, null);
                return ++count == 6;
            }
        });
    }
    ...
    private static void handleLogBalance(final Player player, GameObject object) {
        ...
        AgilityHandler.walk(player, -1, player.getLocation(), player.getLocation().transform(dir.getStepX() * 7, dir.getStepY() * 7, 0), Animation.create(762), 12.0, null);
    }
