...
import org.gielinor.game.node.entity.player.link.BarcrawlManager;
...
public final class BarbarianOutpostCourse extends AgilityCourse {
    ...
    private static final Location[] GRACE_SPAWNS = new Location[]{
        Location.create(2553, 3547, 0),
        Location.create(2538, 3543, 0),
        Location.create(2536, 3545, 1),
        Location.create(2539, 3554, 0),
        Location.create(2542, 3554, 0),
        Location.create(2547, 3554, 0)
    };
    /**
     * The rope delay.
     */
    ...
    public static void dropMark(Player player, int index) {
        if (!AgilityHandler.spawnMarkOfGrace(player, "barb", 35)) {
            return;
        }
    ...
    public boolean handle(final Player player, final Node node, final String option) {
        ...
        switch (id) {
            ...
            case 1948:
                if (player.getLocation().getX() > node.getLocation().getX()) {
                    player.getActionSender().sendMessage("You cannot climb from this side.", 1);
                    break;
                }
                final int flag = node.getLocation().equals(new Location(2536, 3553, 0)) ? 4 :
                    node.getLocation().equals(new Location(2539, 3553, 0)) ? 5 : 6;
    ...
    private void handlePipe(final Player player, final GameObject object) {
        if (player.getSkills().getStaticLevel(Skills.AGILITY) < 35) {
            player.getDialogueInterpreter().sendPlaneMessage("You need an Agility level of at least 35 to enter.");
            return;
        }
        AgilityHandler.forceWalk(player, 0, player.getLocation(), player.getLocation().transform(0, getPipeTransform(player), 0), Animation.create(746), 25, 0, null, 1);
    ...
    private void handleLogBalance(final Player player, final GameObject object) {
        ...
        if (failed) {
            ...
            World.submit(getSwimPulse(player));
            return;
        }
    }
            ...
            public boolean pulse() {
                switch (++counter) {
                    case 6:
                        ...
                        player.getActionSender().sendMessages("...You loose your footing and fall into the water.", "Something in the water bites you.");
                        break;
                    case 7:
                        ...
                        AgilityHandler.walk(player, -1, Location.create(2545, 3545, 0), Location.create(2545, 3543, 0), Animation.create(772), 10, null);
                        break;
                    case 11:
    ...
    public void configure() {
        ...
        ObjectDefinition.forId(23131).getConfigurations().put("option:swing-on", this);
        ObjectDefinition.forId(23144).getConfigurations().put("option:walk-across", this);
        ObjectDefinition.forId(20211).getConfigurations().put("option:climb-over", this);
        ObjectDefinition.forId(23547).getConfigurations().put("option:walk-across", this);
            ...
            switch (stage) {
                ...
                case 1:
                    switch (optionSelect.getButtonId()) {
                        case 1:
                            player("I want to come through his gate.");
                            stage = 5;
                            break;
                        case 2:
                ...
                case 10:
                    ...
                    interpreter.sendPlaneMessage("The guard hands you a Barcrawl card.");
                    stage++;
                    break;
                case 11:
                ...
                case 24:
                    ...
                    interpreter.sendPlaneMessage("You give the card to the barbarian.");
                    stage++;
                    break;
                case 25:
