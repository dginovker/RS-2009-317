...
public final class AgilityPyramidCourse extends AgilityCourse {
    ...
    public void configure() {
        ObjectDefinition.forId(10852).getConfigurations().put("option:climb", this);
    ...
    /**
     * Checks if a player can cross an obstacle.
     *
     * @param g The game object.
     * @param l The player's location.
     * @return <code>True</code> if so.
     */
    public boolean canCross(GameObject g, Location l) {
        switch (g.getId()) {
            //3370, 2843
            case 10859:
                if (g.getLocation().equals(3356, 2847, 2) && l.getY() >= 2847) {
                    return false;
                }
                if (g.getLocation().equals(3364, 2833, 2) && l.getX() < 3365) {
                    return false;
                }
                if (g.getLocation().equals(3370, 2841, 3) && l.getY() < 2842) { // LEFT OFF HERE
                    return false;
                }
                return true;
            case 10860:
                if (g.getLocation().equals(3366, 2851, 1)) {
                    return false;
                }
                if (g.getLocation().equals(3372, 2837, 2)) {
                    return false;
                }
                return true;
            case 10861:
            case 10883:
                if (g.getLocation().equals(3367, 2832, 1) || g.getLocation().equals(3368, 2831, 1)) {
                    return false;
                }
                return true;
            case 10862:
                if (g.getLocation().equals(3362, 2849, 2)) {
                    return false;
                }
                return true;
            case 10867:
                if (g.getLocation().equals(3375, 2840, 1)) {
                    return false;
                }
                return true;
            case 10885:
                if (g.getLocation().equals(3356, 2839, 2)) {
                    return false;
                }
                return true;
            case 10887:
                if (g.getLocation().equals(3360, 2831, 1)) {
                    return false;
                }
                return true;
            case 10889:
                if (g.getLocation().equals(3358, 2845, 3)) {
                    return false;
                }
                return true;
        }
    ...
    public boolean handle(final Player player, Node node, String option) {
        ...
        if (!canCross(object, player.getLocation())) {
        ...
        switch (object.getId()) {
            ...
            case 10856:
                ...
                player.getDialogueInterpreter().sendPlaneMessage("You climb down the steep passage. It leads to the base of the", "pyramid.");
    ...
    private void handleRockClimb(final Player player, final GameObject object) {
        ...
        final Location end = object.getLocation().transform(scale ? 5 : -5, 0, 0);
        if (player.getLocation().getX() <= 3343 && player.getSkills().getStaticLevel(Skills.AGILITY) < 30) {
            player.getActionSender().sendMessage("You must be level 30 agility or higher to climb down the rocks.");
            return;
        }
        if (!scale) {
    ...
    private void handleLowWall(final Player player, final GameObject object) {
        ...
        if (player.getLocation().equals(Location.create(3355, 2848, 1)) || player.getLocation().equals(Location.create(3359, 2838, 3))) {
            ...
            d = Direction.SOUTH;
        } else if (player.getLocation().equals(Location.create(3046, 4694, 2)) || player.getLocation().equals(Location.create(3355, 2850, 1)) || player.getLocation().equals(Location.create(3041, 4702, 2))) {
            d = Direction.EAST;
        ...
        player.getActionSender().sendMessage("You climb the low wall...", 1);
        if (fail) {
    ...
    private void handleLedge(final Player player, final GameObject object) {
        ...
        player.getActionSender().sendMessage("You put your foot on the ledge and try to edge across...", 1);
        if (fail) {
            player.lock(4);
    ...
    private void handlePlank(final Player player, final GameObject object) {
        ...
        AgilityHandler.walk(player, fail ? -1 : 1, player.getLocation(), end, Animation.create(762), fail ? 0.0 : 15.00, fail ? null : "You walk carefully across the slippery plank...");
        if (fail) {
    ...
    private void handleJumpGap(Player player, GameObject object) {
        ...
        final boolean fail = hasFailed(player);
        player.getActionSender().sendMessage("You jump the gap...", 1);
        if (fail) {
    ...
    private void handleGapCross(final Player player, final GameObject object) {
        ...
        AgilityHandler.walk(player, 4, player.getLocation(), end, animation, 56.4, null);
    }
    ...
    private void handlePyramidTop(final Player player, final GameObject object) {
        ...
        World.submit(new Pulse(3, player) {
            ...
            public boolean pulse() {
                if (player.getConfigManager().get(640) == 505 || player.getConfigManager().get(640) == 515) {
                    player.getActionSender().sendMessage("You find nothing on top of the pyramid.");
                    return true;
                }
