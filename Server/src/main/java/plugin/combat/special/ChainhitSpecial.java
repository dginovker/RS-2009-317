 ...
 * @author Emperor
 */
public final class ChainhitSpecial extends RangeSwingHandler implements Plugin<Object> {
    ...
    @Override public int swing(Entity entity, final Entity victim, BattleState state) {
        if (entity.getAttribute("chain-hit_v") != null) {
            ((Player) entity).getActionSender().sendMessage("You're already using the chain-hit special attack.");
     ...
     */
    private void handleHit(final Entity entity, final Entity victim, final Player player, final BattleState state) {
        World.submit(new Pulse(1, player, victim) {

            @Override
            public boolean pulse() {
                ChainhitSpecial.super.onImpact(player, victim, state);
                ChainhitSpecial.super.impact(entity, victim, state);
                return true;
            }
        });
        ...
        if (victim.getProperties().isMultiZone() && player.getSettings().getSpecialEnergy() >= SPECIAL_ENERGY) {
            ...
            for (Iterator<? extends Entity> it = list.iterator(); it.hasNext(); ) {
                ...
                World.submit(new Pulse((int) (distance / 3), entity, victim, e) {

                    @Override
                    public boolean pulse() {
                        BattleState bs = new BattleState(entity, e);
                        bs.setMaximumHit(calculateHit(player, e, 1.0));
                        bs.setEstimatedHit(RandomUtil.RANDOM.nextInt(bs.getMaximumHit()));
                        handleHit(victim, e, player, bs);
                        ChainhitSpecial.super.visualizeImpact(player, e, bs);
                        return true;
                    }
                });
