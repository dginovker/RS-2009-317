...
public final class DagannothKingNPC extends AbstractNPC {
    ...
    public void sendImpact(BattleState state) {
        if (state.getEstimatedHit() > type.getMaxHit()) {
            state.setEstimatedHit(RandomUtil.random(type.getMaxHit() - 5, type.getMaxHit()));
        }
        if (type != DagType.REX && RandomUtil.random(5) <= 2) {
            List<Player> players = RegionManager.getLocalPlayers(this, 9);
            if (players.size() <= 1) {
                return;
            }
            Player newP = players.get(RandomUtil.random(players.size()));
            if (newP != null) {
    ...
    public void finalizeDeath(Entity killer) {
        ...
        if (killer instanceof Player) {
            switch (getId()) {
                case 2881:
                    killer.asPlayer().getSavedData().getBossKillLog().increaseDagannothSupremeKills(1);
                    break;
                case 2882:
                    killer.asPlayer().getSavedData().getBossKillLog().increaseDagannothPrimeKills(1);
                    break;
                case 2883:
                    killer.asPlayer().getSavedData().getBossKillLog().increaseDagannothRexKills(1);
                    break;
            }
        }
    }
