...
public final class SpinolypNPC extends AbstractNPC {
    ...
    public void sendImpact(BattleState state) {
        if (state.getEstimatedHit() == 0 && RandomUtil.random(15) < 2) {
            state.setEstimatedHit(RandomUtil.random(1, 11));
        }
        if (state.getEstimatedHit() > 11) {
            state.setEstimatedHit(RandomUtil.random(3, 9));
        }
    }

    @Override
    public int[] getIds() {
        return new int[]{ 5947, 5961, 6963 };
    }
    ...
    public static final class SpinolpySwingHandler extends MultiSwingHandler {
        ...
        /**
         * Constructs a new {@Code SpinolpySwingHandler} {@Code Object}
         */
