...
public final class KalphiteQueenNPC extends AbstractNPC {
    ...
    public void finalizeDeath(Entity killer) {
        if (getId() == 1160) {
            ...
            if (killer instanceof Player) {
                killer.asPlayer().getSavedData().getBossKillLog().increaseKalphiteQueenKills(1);
            }
            return;
        }
        ...
        lock(TRANSFORM_ANIM.getDefinition().getDurationTicks() + 1);
    }
    ...
    static class KQCombatSwingHandler extends CombatSwingHandler {
        ...
        public int swing(Entity entity, Entity victim, BattleState state) {
            style = super.getType();
            super.setType(CombatStyle.values()[1 + RandomUtil.RANDOM.nextInt(2)]);
            int ticks = 1;
            if (CombatStyle.MELEE.getSwingHandler().canSwing(entity, victim) != InteractionType.NO_INTERACT && RandomUtil.random(10) < 4) {
        ...
        public void impact(Entity entity, Entity victim, BattleState battleState) {
            CombatStyle style = battleState.getStyle();
            if (style == CombatStyle.RANGE && battleState.getTargets() != null) {
                for (BattleState s : battleState.getTargets()) {
                    handleExtraEffects(victim, s);
        ...
        private void handleMagicImpact(final Entity e, final Entity last, final Entity victim, final BattleState[] targets, int index) {
            ...
            BattleState bs = targets[index];
            if (bs == null) {
                return;
            }
            int hit = 0;
            if (CombatStyle.MAGIC.getSwingHandler().isAccurateImpact(e, victim)) {
                hit = RandomUtil.RANDOM.nextInt(32);
            }
            bs.setEstimatedHit(hit);
            handleExtraEffects(victim, bs);
            CombatStyle.MAGIC.getSwingHandler().adjustBattleState(e, victim, bs);
            victim.getImpactHandler().handleImpact(e, bs.getEstimatedHit(), CombatStyle.MAGIC, bs);
            CombatStyle.MAGIC.getSwingHandler().onImpact(e, victim, bs);
            while (++index < targets.length && (bs = targets[index]) == null) {
                ;
            }
            if (bs == null || index >= targets.length) {
                return;
            }
            final BattleState t = bs;
            final int targetIndex = index;
            t.getVictim().graphics(MAGIC_END_GFX);
            Projectile.create(victim, t.getVictim(), 280, 41, 36, 0, 30, 15, 11).send();
            World.submit(new Pulse(1, t.getVictim()) {
