...
public class SpiritSpiderNPC extends Familiar {
    ...
    private void createEggs() {
        final int amount = RandomUtil.random(8);
        ...
        for (int i = 0; i < amount; i++) {
            ...
            owner.getActionSender().sendPositionedGraphic(1342, 0, 0, location);
    ...
    private Location getEggLocation() {
        Location loc = owner.getLocation().transform(RandomUtil.random(2) == 1 ? -RandomUtil.random(2) : RandomUtil.random(2), RandomUtil.random(2) == 1 ? -RandomUtil.random(2) : RandomUtil.random(2), 0);
