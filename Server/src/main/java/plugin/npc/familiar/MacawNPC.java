...
public class MacawNPC extends Forager {
    ...
    @Override
    public Plugin<Object> newInstance(Object object) {
    ...
    protected boolean specialMove(FamiliarSpecial special) {
        if (specialDelay > World.getTicks()) {
            owner.getActionSender().sendMessage("You must wait one minute until using the macaws special again.");
            return false;
        }
        final Item herb = HERBS[RandomUtil.random(HERBS.length)];
        animate(Animation.create(8013));
        graphics(Graphics.create(1321), 2);
        World.submit(new Pulse(5, owner) {
