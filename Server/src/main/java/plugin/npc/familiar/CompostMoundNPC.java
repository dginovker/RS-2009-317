...
public class CompostMoundNPC extends Forager {
    ...
    public CompostMoundNPC(Player owner, int id) {
        super(owner, id, 2400, 12091, 12, ITEMS);
    }
    ...
    protected boolean specialMove(FamiliarSpecial special) {
        final GameObject gameObject = (GameObject) special.getNode();
        if (!gameObject.getName().equals("Compost Bin")) {
            owner.getActionSender().sendMessage("This scroll can only be used on an empty compost bin.");
            return false;
        }
        final CompostBin bin = owner.getFarmingManager().getCompostManager().getBin(gameObject);
        if (bin.getState(owner) != 0) {
            owner.getActionSender().sendMessage("This scroll can only be used on an empty compost bin.");
            return false;
        }
        final boolean superCompost = RandomUtil.random(10) == 1;
        faceLocation(gameObject.getLocation());
            ...
            switch (stage) {
                ...
                case 31:
                    if (player.getSkills().getLevel(Skills.FARMING) > player.getSkills().getStaticLevel(Skills.FARMING)) {
                        ...
                        player.getActionSender().sendMessage("Your stat cannot be boosted this way right now.");
                        return true;
                    }
                    player.getSkills().updateLevel(Skills.FARMING, (int) (1 + (player.getSkills().getStaticLevel(Skills.FARMING) * 0.02)));
                    player.getActionSender().sendMessage("The Compost mound has boosted your Farming stat.");
                    end();
                    break;
            }
            return true;
