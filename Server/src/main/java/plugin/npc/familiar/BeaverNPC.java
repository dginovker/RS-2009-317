...
public class BeaverNPC extends Forager {
    ...
    protected boolean specialMove(FamiliarSpecial special) {
        ...
        if (!isTree(object.getName())) {
            owner.getActionSender().sendMessages("This scroll only works on naturally growing, oak, willow, arctic pine", "teak, mahogany, maple, yew, and magic trees.");
            return false;
        }
        ...
        if (object.getLocation().getDistance(getLocation()) > 5) {
            owner.getActionSender().sendMessages("The beaver is a little too far from the tree for the scroll to work - stand", "closer.");
            return false;
        }
        ...
        World.submit(new Pulse(ticks, owner, this) {
            ...
            public boolean pulse() {
                ...
                World.submit(new Pulse(1, owner, BeaverNPC.this) {
                    ...
                    public boolean pulse() {
                        switch (++counter) {
                            default:
                                if (counter > 3) {
                                    if (RandomUtil.random(12) < 4) {
                                        owner.getInventory().add(ITEMS[RandomUtil.random(ITEMS.length)], owner);
                                        recieved = true;
                                    }
                                }
                                break;
