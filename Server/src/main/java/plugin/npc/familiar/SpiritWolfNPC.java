...
public final class SpiritWolfNPC extends Familiar {
    ...
    protected boolean specialMove(FamiliarSpecial special) {
        if (!(special.getNode() instanceof NPC)) {
            owner.getActionSender().sendMessage("You can only target monsters with this special move.");
            return false;
        }
        final NPC npc = (NPC) special.getNode();
        if (npc.getWalkRadius() > 20) { //Usually indicates special NPC.
            owner.getActionSender().sendMessage("This monster won't get intimidated by your familiar.");
            return false;
        }
        if (cutscene != null) {
            if (npc.getId() != 6990) {
                owner.getActionSender().sendMessage("You can't do this right now.");
                return false;
            }
        ...
        World.submit(new Pulse(2, this, npc) {
