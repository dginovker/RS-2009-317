...
import org.gielinor.game.node.entity.combat.equipment.FireType;
...
public final class KingBlackDragonNPC extends AbstractNPC {
    ...
    public void finalizeDeath(Entity killer) {
        ...
        if (killer instanceof Player) {
            if (RandomUtil.random(4500) == 1 && killer.asPlayer().getEquipment().contains(2572, 1)) {
                GroundItemManager.create(new Item(1543), getLocation(), ((Player) killer));
                ((Player) killer).getActionSender().sendMessage("<col=FF7000><shad=000>Your ring of wealth shines darkly...");
                Graphics.send(Graphics.create(671), getLocation());
            }
            if (RandomUtil.random(3000) == 1) {
                GroundItemManager.create(new Item(11286), getLocation(), (Player) killer);
            }
        }
    }
    ...
    static class KBDCombatSwingHandler extends CombatSwingHandler {
        ...
        public int swing(Entity entity, Entity victim, BattleState state) {
            ...
            int hit = 0;
            int ticks = 1;
            if (victim.getCenterLocation().withinDistance(entity.getCenterLocation(), getCombatDistance(entity, victim, 1)) && RandomUtil.random(10) < 7) {
                style = CombatStyle.MELEE;
            } else {
                ticks += (int) Math.ceil(entity.getLocation().getDistance(victim.getLocation()) * 0.3);
            }
            fireType = FireType.values()[RandomUtil.random(FireType.values().length)];
