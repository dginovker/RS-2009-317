...
public class SeerBartenderDialogue extends DialoguePlugin {
    ...
    public void buy(int item, int ammount) {
        ...
        if (!player.getInventory().contains(Item.COINS, ammount)) {
            interpreter.sendDialogues(player, FacialExpression.NO_EXPRESSION, "Sorry, I don't seem to have enough coins.");
            stage = 99;
        } else {
            end();
            player.getInventory().remove(new Item(Item.COINS, ammount));
            player.getInventory().add(new Item(item, 1));
            if (item == 1917) {
                player.getActionSender().sendMessage("You buy a beer.");
            } else if (item == 2327) {
                player.getActionSender().sendMessage("You buy a nice hot meat pie.");
            } else {
                player.getActionSender().sendMessage("You buy a bowl of home made stew.");
            }
        }
    }
