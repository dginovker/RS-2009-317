...
public final class GodswordDialogue extends DialoguePlugin {
    ...
    public boolean open(Object... args) {
        used = (int) args[0];
        interpreter.sendPlaneMessage("You set to work, trying to fix the ancient sword.");
        return true;
    }
            ...
            case 0:
                ...
                if (!passBlade) {
                    if (player.getInventory().remove(new Item(used)) && player.getInventory().remove(new Item(remove))) {
                        ...
                        interpreter.sendPlaneMessage("Even for an experienced smith it is not an easy task, but eventually", "it is done.");
                ...
                if (base == -1) {
                    ...
                    interpreter.sendPlaneMessage("You didn't have all the required items.");
                    return true;
                }
