...
import org.gielinor.game.content.global.BirdsNest;
...
public final class WysonTheGardenerDialogue extends DialoguePlugin {
     ...
     */
    private static final Item[] COINS = new Item[]{ new Item(Item.COINS, 15), new Item(Item.COINS, 20) };
    ...
    public boolean handle(int interfaceId, OptionSelect optionSelect) {
        ...
        int moleSkin = 0;
        if (birdNest) {
            moleClaws = player.getInventory().getCount(MOLE_CLAW);
            moleSkin = player.getInventory().getCount(MOLE_SKIN);
        }
        switch (stage) {
            case 0:
            ...
            case 133:
                ...
                break;
            case 131:
                if (player.getInventory().contains(Item.COINS, 15)) {
                    ...
                    player.getActionSender().sendMessage("You buy a woad leaf from Wyson.");
                    stage = 132;
                } else {
                    end();
                    player.getActionSender().sendMessage("You need 15 cold coins to buy a woad leaf.");
                }
                break;
            case 132:
            ...
            case 140:
                ...
                break;
            case 141:
                if (player.getInventory().contains(Item.COINS, 20)) {
                    player.getInventory().remove(COINS[1]);
                    for (int i = 0; i < 2; i++) {
                        player.getInventory().add(WOAD_LEAF, player);
                    }
                    interpreter.sendDialogues(player, FacialExpression.NORMAL, "Thanks.");
                    player.getActionSender().sendMessage("You buy two woad leaves from Wyson.");
                    stage = 132;
                } else {
                    end();
                    player.getActionSender().sendMessage("You need 15 cold coins to buy a woad leaf.");
    ...
    private void addNests(int amount) {
        player.getInventory().add(new Item(BirdsNest.SEED.getNest().getId(), amount));
    }
