...
public final class DragonShieldDialogue extends DialoguePlugin {
    ...
    public boolean open(Object... args) {
        type = (int) args[0];
        if (type == 1) {
            interpreter.sendPlaneMessage("You set to work trying to fix the ancient shield. It's seen some", "heavy action and needs some serious work doing to it.");
            stage = 0;
        } else {
            interpreter.sendPlaneMessage("You set to work, trying to attach the ancient draconic", "visage to your anti-dragonbreath shield. It's not easy to", "work with the ancient artifact and it takes all of your", "skills as a master smith.");
            stage = 10;
        }
        return true;
    }
    ...
    public boolean handle(int interfaceId, OptionSelect optionSelect) {
        switch (stage) {
            case 0:
                ...
                interpreter.sendPlaneMessage("Even for an experienced armourer it is not an easy task, but", "eventually it is ready. You have restored the dragon square shield to", "its former glory.");
            ...
            case 10:
                player.lock(5);
                player.animate(ANIMATION);
                interpreter.sendPlaneMessage("Even for an experienced armourer it is not an easy task", "but eventually it is ready. You have crafted the", "draconic visage and anti-dragonbreath shield into a", "dragonfire shield.");
