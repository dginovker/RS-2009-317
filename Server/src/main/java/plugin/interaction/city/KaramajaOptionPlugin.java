...
public final class KaramajaOptionPlugin extends OptionHandler {
     ...
     */
    private static final Component TICKET_EXCHANGE = new Component(8292);
    ...
    public Plugin<Object> newInstance(Object arg) {
        ...
        ObjectDefinition.forId(2230).getConfigurations().put("option:board", this);//cart travel.
        ObjectDefinition.forId(2230).getConfigurations().put("option:pay-fare", this);//cart travel.
        NPCDefinition.forId(1178).getConfigurations().put("option:fish", this);//lubufu fishing spot
        ObjectDefinition.forId(5083).getConfigurations().put("option:enter", this);//sanibo dungeon entrance.
        ObjectDefinition.forId(9038).getConfigurations().put("option:open", this);//trading sticks door.
    ...
    public boolean handle(Player player, Node node, String option) {
        switch (option) {
            ...
            case "trade":
                switch (node.getId()) {
                    case 1055:
                        player.getInterfaceState().open(TICKET_EXCHANGE);
                        break;
                }
                break;
            case "fish":
            ...
            case "pay":
                if (!player.getAttribute("capn_izzy", false)) {
                    if (player.getInventory().contains(Item.COINS, 200) && player.getInventory().remove(new Item(Item.COINS, 200))) {
                        player.getDialogueInterpreter().sendItemMessage(new Item(Item.COINS, 200), "You give Cap'n Izzy the 200 coin entrance fee.");
                        player.getActionSender().sendMessage("You give Cap'n Izzy the 200 coin entrance fee.");
                        player.saveAttribute("capn_izzy", true);
                        return true;
                    } else {
                        player.getActionSender().sendMessage("You don't have the 200 coin entrance fee.");
                    }
                } else {
            ...
            case "pay-fare":
                switch (node.getId()) {
                    case 2230:
                        player.getActionSender().sendMessage("This cart appears to have been out of action for some time.");
                        break;
                }
                break;
            case "board":
                switch (node.getId()) {
                    case 2230:
                        player.getDialogueInterpreter().open(510, Repository.findNPC(510), true);
                        break;
                }
                break;
            case "climb-up":
            ...
            case "climb-down":
                switch (node.getId()) {
                    case 492:
                        player.getActionSender().sendMessage("You climb down through the pot hole.");
            ...
            case "climb":
                switch (node.getId()) {
                    case 1764:
                        ...
                        player.getActionSender().sendMessage("You climb up the hanging rope...");
                        player.getActionSender().sendMessage("You appear on the volcano rim.");
                        break;
                }
                break;
            case "open":
                switch (node.getId()) {
                    case 2626:
                        ...
                        break;
                    case 2628:
                        player.getActionSender().sendMessage("The door is securely closed.");
                        break;
                    case 2627:
                        player.getDialogueInterpreter().open(788, Repository.findNPC(788), true);
                        break;
                    case 1591:
                        player.getActionSender().sendMessage("You try and open the door...");
                        player.getActionSender().sendMessage("The door is locked tight, I can't open it.");
                        break;
                    case 9038:
                        player.getActionSender().sendMessage("The door is securely closed.");
                        break;
                    case 2439:
                        player.getActionSender().sendMessage("The gate doesn't open.");
                        break;
                }
                break;
            case "search":
                switch (node.getId()) {
                    ...
                    case 2072:
                        ...
                        if (amt >= 10) {
                            player.getActionSender().sendMessage("The crate is full of bananas.");
                            return true;
                        }
                        if (amt == 0) {
                            player.getActionSender().sendMessage("The crate is completely empty.");
                        } else {
                            player.getActionSender().sendMessage("The crate has " + amt + " banana " + (amt > 1 ? "s" : "") + "inside.");
                        }
                        break;
                }
                break;
            case "fill":
                if (player.getSavedData().getGlobalData().isLuthasTask()) {
                    int bananas = player.getInventory().getCount(BANANA);
                    if (bananas > 0) {
                        player.getDialogueInterpreter().sendPlaneMessage("You pack all your bananas into the crate.");
                        if (player.getInventory().remove(new Item(BANANA.getId(), bananas))) {
                            player.getSavedData().getGlobalData().setKaramjaBananas(bananas + player.getSavedData().getGlobalData().getKaramjaBananas());
                        }
                    }
                } else {
                    player.getActionSender().sendMessage("I don't know what goes in there.");
                }
                break;
        }
        return true;
