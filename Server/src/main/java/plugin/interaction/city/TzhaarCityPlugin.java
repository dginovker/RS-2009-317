...
public final class TzhaarCityPlugin extends OptionHandler {
    ...
    public Plugin<Object> newInstance(Object arg) throws Throwable {
        ObjectDefinition.forId(31284).getConfigurations().put("option:enter", this);//karamja cave.
        ObjectDefinition.forId(9359).getConfigurations().put("option:enter", this);//tzhaar exit
        ObjectDefinition.forId(11833).getConfigurations().put("option:enter", this);
        ObjectDefinition.forId(11846).getConfigurations().put("option:pass", this);
    ...
    public boolean handle(Player player, Node node, String option) {
        ...
        switch (option) {
            case "enter":
                switch (id) {
                    case 31284:
                        player.setTeleportTarget(LOCATIONS[0]);
                        break;
                    case 9359:
                        player.setTeleportTarget(LOCATIONS[1]);
                        break;
                    case 11833:
                        if (player.getFamiliarManager().hasFamiliar()) {
                            player.getActionSender().sendMessage("You can't enter this with a follower.");
                            break;
                        }
                ...
                case 1:
                    switch (buttonId) {
                        case 1:
                            ...
                            Shops.forId(npc.getId()).open(player);
                            break;
                        case 2:
