        ...
        switch (object.getId()) {
            ...
            case 5107:
                ...
                if (player.getSkills().getLevel(Skills.WOODCUTTING) < level) {
                    player.getActionSender().sendMessage("You need a woodcutting level of " + level + " to chop down this vine.");
                    return true;
                }
                SkillingTool tool = SkillingTool.getHatchet(player);
                if (tool == null) {
                    player.getActionSender().sendMessage("You don't have an axe to cut these vines.");
                    return true;
                }
            ...
            case 5111:
                if (player.getSkills().getLevel(Skills.AGILITY) < 12) {
                    player.getActionSender().sendMessage("You need an agility level of 12 to cross this.");
                    return true;
                }
                player.lock(12);
            ...
            case 5088:
                if (player.getSkills().getLevel(Skills.AGILITY) < 30) {
                    player.getActionSender().sendMessage("You need an agility level of 30 to cross this.");
                    return true;
                }
            ...
            case 5100:
                ...
                if (player.getSkills().getLevel(Skills.AGILITY) < level) {
                    player.getActionSender().sendMessage("You need an agility level of " + level + " to squeeze through the pipe.");
                    return true;
                }
