...
public final class PortSarimPlugin extends OptionHandler {
    ...
    public Plugin<Object> newInstance(Object arg) throws Throwable {
        NPCDefinition.forId(1099).getConfigurations().put("option:talk-to", this);
...
//                    player.getActionSender().sendMessage("The goblin is already in prison. You have no reason to attack him.");
//                } else {
...
//                }
            ...
            case "open":
                switch (((GameObject) node).getId()) {
                    case 9565:
                    case 9563:
                        player.getActionSender().sendMessage("The door is securely locked.");
                        break;
                }
                break;
            case "pick-lock":
                switch (((GameObject) node).getId()) {
                    case 9565:
                        if (player.getLocation().getY() <= 3187) {
                            player.getActionSender().sendMessage("You simply cannot find a way to pick the lock from this side.");
                            return true;
                        }
                        break;
                }
            ...
            case "talk-to":
                player.lock(2);
                ((NPC) node).sendChat(MESSAGES[RandomUtil.random(MESSAGES.length)]);
            ...
            case "search"://banan box
                ...
                player.getActionSender().sendMessage("There are lots of bananas in the crate.");
                player.lock(2);
                World.submit(new Pulse(1) {
