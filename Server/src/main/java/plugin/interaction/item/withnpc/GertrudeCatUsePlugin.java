...
public final class GertrudeCatUsePlugin extends UseWithHandler {
    ...
    public boolean handle(NodeUsageEvent event) {
        ...
        if (event.getUsedItem().getId() == 1927 && quest.getStage() == 20) {
            ...
            player.getDialogueInterpreter().sendPlaneMessage("The cat doesn't seem interested in that.");
        } else if (event.getUsedItem().getId() == 1555) {
            if (player.getInventory().remove(event.getUsedItem())) {
                quest.setStage(60);
                player.lock(5);
                player.getDialogueInterpreter().sendPlaneMessage(false, "The cats scurry off back to Gertrude...");
