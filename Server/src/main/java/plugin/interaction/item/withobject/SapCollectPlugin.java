...
public final class SapCollectPlugin extends UseWithHandler {
    ...
    public boolean handle(NodeUsageEvent event) {
        ...
        if (!player.getInventory().remove(BUCKET)) {
            player.getActionSender().sendMessage("I need an empty bucket to collect the sap.");
            return true;
        }
        player.animate(ANIMATION);
        player.getInventory().remove(BUCKET);
        player.getInventory().add(BUCKET_OF_SAP);
        player.getActionSender().sendMessage("You cut the tree and allow its sap to drip down into your bucket.");
        return true;
    }
    ...
    public static final class SapEmptyPlugin extends OptionHandler {
        ...
        public boolean handle(Player player, Node node, String option) {
            ...
            player.getActionSender().sendMessage("You empty the contents of the bucket on the floor.");
            return true;
        }
