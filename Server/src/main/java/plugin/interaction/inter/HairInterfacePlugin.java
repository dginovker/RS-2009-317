...
public final class HairInterfacePlugin extends ComponentPlugin {
     ...
     */
    private static final Item COINS = new Item(Item.COINS, 1000);
...
//		ComponentDefinition.put(199, this);
    ...
    public boolean handle(Player player, Component component, int opcode, int button, int slot, int itemId) {
        handle(player, button, component.getId() == 204, component.getId() == 199 ? new int[]{ 1, 2 } : new int[]{ 1 });
        return true;
    }
        ...
        if (inter != null && inter.length == 2) {
            ...
            switch (button) {
                ...
                case 138:
                    ...
                    break;
                case 104:
                    confirm(player);/**confirms the players choice on design.*/
                    break;
            }
            if (style != -1) {
    ...
    public static void confirm(Player player) {
        ...
        if (player.getInventory().remove(COINS)) {
            if (player.getAttribute("newHair") != null) {
                player.getAppearance().getHair().setLook(player.getAttribute("newHair"));
            }
            if (player.getAttribute("newHairColour") != null) {
                player.getAppearance().getHair().setColour(player.getAttribute("newHairColour"));
            }
            if (player.getAttribute("newBeard") != null) {
                player.getAppearance().getBeard().setLook(player.getAttribute("newBeard"));
            }
