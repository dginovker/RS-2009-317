        ...
        switch (option) {
            case "open":
                ...
                if (player.getAttribute("shantay-jail", false) && player.getLocation().getX() > 3299) {
                    player.removeAttribute("shantay-jail");// if we tele out(witch is allowed) then we need to remove.
                }
            ...
            case "bribe":
                ...
                break;
            case "look-at":
                player.getDialogueInterpreter().sendPlaneMessage("<col=8A0808>The Desert is a VERY Dangerous place. Do not enter if you are", "<col=8A0808>afraid of dying. Beware of high temperatures, and storms, robbers,", "<col=8A0808>and slavers. No responsibility is taken by Shantay if anything bad", "<col=8A0808>should happen to you in any circumstances whatsoever.");
                break;
            case "go-through":
                ...
                player.getInterfaceState().open(new Component(565));
                break;
            case "quick-pass":
                if (player.getLocation().getY() < 3117) {
                    AgilityHandler.walk(player, 0, player.getLocation(), player.getLocation().transform(0, player.getLocation().getY() > 3116 ? -2 : 2, 0), null, 0, null);
            ...
            switch (button) {
                case 17:// proceed.
                    ...
                    if (!player.getInventory().containsItem(PASS)) {
                        player.getDialogueInterpreter().sendDialogues(4648, null, "You need a Shantay pass to get through this gate. See", "Shantay, he will sell you one for a very reasonable", "price.");
                        return true;
                    } else {
                        player.getDialogueInterpreter().open(4648, null, true);
                    }
                    break;
                case 18://stay out.
                    player.getInterfaceState().close();
                    player.getDialogueInterpreter().sendPlaneMessage("You decide that your visit to the desert can be postponed.", "Perhaps indefinitely.");
                    break;
            }
            return true;
        }
