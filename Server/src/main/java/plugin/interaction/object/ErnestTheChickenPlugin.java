...
public final class ErnestTheChickenPlugin extends OptionHandler {
    ...
    public static class LeverCacheExtension {
        ...
        public final void pull(final Lever lever, final GameObject object) {
            ...
            World.submit(new Pulse(1) {
                ...
                public boolean pulse() {
                    updateConfigs();
                    player.getActionSender().sendMessage("You pull lever " + lever.name().replace("LEVER_", "").trim() + " " + (up ? "down" : "up") + ".");
        ...
        public final void inspect(final Lever lever) {
            player.getActionSender().sendMessage("The lever is " + (isUp(lever) ? "up" : "down") + ".");
        }
        ...
        public final void walk(final GameObject object) {
            player.lock(4);
            World.submit(new Pulse(1, player, object) {
                ...
                public boolean pulse() {
                    ...
                    final GameObject second = DoorActionHandler.getSecondDoor(object);
        ...
        public final int calculateDoorConfig() {
            ...
            for (int i = 0; i < levers.length; i++) {
                ...
                if (downCount == 0 || downCount == 1 && !isUp(Lever.LEVER_B) && lever == Lever.LEVER_B && levers[0]) {//translation: no down lever and lever b is on/off just send 0.
                    value = 0;
                    break;
                }
