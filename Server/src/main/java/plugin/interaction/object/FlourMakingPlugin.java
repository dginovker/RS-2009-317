...
public final class FlourMakingPlugin extends OptionHandler {
    ...
    public boolean handle(Player player, Node node, String option) {
        ...
        switch (option) {
            case "operate":
                ...
                player.getActionSender().sendSound(new Audio(3189));
                if (extension.getSemiCharges() < 1) {
                    player.getActionSender().sendMessage("You operate the empty hopper. Nothing interesting happens.");
                } else {
                    extension.emptyChute();
                    player.getActionSender().sendMessage("You operate the hopper. The grain slides down the chute.");
     ...
     * @return <code>True</code> if emptied.
     */
    private boolean empty(final Player player, final FlourExtension extension) {
        ...
        if (!player.getInventory().containsItem(EMPTY_POT)) {
            player.getActionSender().sendMessage("I need an empty pot to hold the flour in.");
            return true;
        }
        if (player.getInventory().remove(EMPTY_POT)) {
            player.getInventory().add(FLOUR);
            extension.decrement(1);
            player.getActionSender().sendMessage(!extension.isEmpty() ? "You fill a pot with flour from the bin." : "You fill a pot with the last of the flour in the bin.");
            if (extension.getCharges() < 1) {
    ...
    public static final class GrainHopperPlugin extends UseWithHandler {
        ...
        public boolean handle(NodeUsageEvent event) {
            ...
            if (extension.getSemiCharges() > 0) {
                player.getActionSender().sendMessage("There is already grain in the hopper.");
                return true;
            }
            if (player.getInventory().remove(GRAIN)) {
                player.animate(ANIMATION);
                extension.fill();
                player.getActionSender().sendMessage("You put the grain in the hopper.");
            }
            return true;
        }
