...
/**
 * Handles the pick option of interactive scenery (non-farming cabbages, potatoes, bananas, etc)
 *
 * @author Emperor
 * @version 1.0
 */
public final class FieldPickingPlugin extends OptionHandler {
    ...
    public boolean handle(final Player player, Node node, String option) {
        ...
        final Item reward = new Item(plant == PickingPlant.POTATO && RandomUtil.random(10) == 0 ? 5318 : plant.reward);
        ...
        if (plant.name().startsWith("NETTLES") && (player.getEquipment().get(Equipment.SLOT_HANDS) == null || player.getEquipment().get(Equipment.SLOT_HANDS) != null && !player.getEquipment().get(Equipment.SLOT_HANDS).getName().contains("glove"))) {
        ...
        World.submit(new Pulse(1, player) {
            ...
            public boolean pulse() {
                ...
                if (!plant.name().startsWith("NETTLES")) {
                    player.getActionSender().sendMessage("You pick a " + reward.getName().toLowerCase() + ".");
                } else {
                    player.getActionSender().sendMessage("You pick a handful of nettles.");
                }
                return true;
            }
        });
    ...
    private void handleFlaxPick(final Player player, final GameObject object, final PickingPlant plant) {
        ...
        player.getActionSender().sendSound(new Audio(2581));
        player.getActionSender().sendMessage("You pick some flax.");
        if (charge > 1000 + RandomUtil.random(2, 8)) {
            object.setActive(false);
