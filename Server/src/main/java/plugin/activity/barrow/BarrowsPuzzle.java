...
public final class BarrowsPuzzle extends ComponentPlugin {
     ...
     */
    private static final Component COMPONENT = new Component(4543);
    ...
    public static void open(Player player, int index) {
        ...
        for (int i = 0; i < puzzle.questionModels.length; i++) {
            PacketRepository.send(DisplayModel.class, new DisplayModelContext(player, ModelType.MODEL, puzzle.questionModels[i] & 0xFFFF, 0, 4545 + i, -1));
        }
        for (int i = 0; i < puzzle.answerModels.length; i++) {
            PacketRepository.send(DisplayModel.class, new DisplayModelContext(player, ModelType.MODEL, puzzle.answerModels[i] & 0xFFFF, 0, 4550 + i, -1));
        }
    }
    ...
    public boolean handle(Player player, Component component, int opcode, int button, int slot, int itemId) {
        switch (button) {
            ...
            case 4552:
                ...
                boolean correct = ((player.getAttribute("puzzle:answers", new int[3])[button - 4550] >> 16) & 0xFF) == 1;
                if (!correct) {
                ...
                player.saveAttribute("barrow:solvedpuzzle", true);
