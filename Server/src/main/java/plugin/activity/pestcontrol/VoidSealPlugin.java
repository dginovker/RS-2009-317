...
public final class VoidSealPlugin extends OptionHandler {
    ...
    public boolean handle(Player player, Node node, String option) {
        if (player.getViewport().getRegion().getRegionId() != 10536) {
            player.getActionSender().sendMessage("You can only use the seal in Pest Control.");
            return true;
        }
        ...
        player.getActionSender().sendMessage("You unleash the power of the Void Knights!");
        Item replace = null;
        if (item.getId() != 11673) {
        ...
        for (NPC npc : RegionManager.getLocalNpcs(player, 2)) {
            if (canTarget(npc)) {
                npc.getImpactHandler().manualHit(player, 7 + RandomUtil.randomize(5), HitsplatType.NORMAL, 1);
    ...
    private static boolean canTarget(NPC npc) {
        return npc instanceof PCDefilerNPC || npc instanceof PCRavagerNPC
            || npc instanceof PCShifterNPC || npc instanceof PCSpinnerNPC
            || npc instanceof PCSplatterNPC || npc instanceof PCTorcherNPC;
    }
