...
public final class PestControlSession {
     ...
     * Updates the session.
     *
     * @return <code>True</code> if the session has finished.
     */
    public boolean update() {
        ...
        if (ticks % 100 == 0) {
            sendString(20 - (ticks / 100) + " min", 24537);
        }
        switch (++ticks) {
    ...
    public void sendString(String message, int child) {
        region.getPlanes()[0].getPlayers().stream().filter(p -> p.isActive()).forEach(p -> {
            p.getActionSender().sendString(message, child);
        });
    }
    ...
    public void addZealGained(Player player, int zeal) {
        ...
        String col = total > 50 ? "<col=00FF00>" : "<col=F80000>";
        if (total > 350) {
            player.getActionSender().sendString(col + "LOTS!", 24536);
        } else {
            player.getActionSender().sendString(col + Integer.toString(total), 24536);
        }
    }
    ...
    public void sendConfig(int id, int value) {
        region.getPlanes()[0].getPlayers().stream().filter(Node::isActive).forEach(p -> {
            p.getActionSender().sendInterfaceConfig(id, value);
        });
    }
    ...
    public void removePortalShield(int index) {
        ...
        for (Player p : region.getPlanes()[0].getPlayers()) {
            if (p.isActive()) {
                p.getActionSender().sendInterfaceConfig(203 + index, 1);
                p.getActionSender().sendMessage(message);
            }
        }
    ...
    public void startGame(List<Player> waitingPlayers) {
        ...
        Collections.shuffle(list, RandomUtil.RANDOM);
        ...
        for (Iterator<Player> it = waitingPlayers.iterator(); it.hasNext(); ) {
            ...
            if (p.getSession().isActive()) {
                if (++count > 25) {
                    ...
                    p.getActionSender().sendMessage("You have been given priority level " + priority + " over other players in joining the next");
                    p.getActionSender().sendMessage("game.");
    ...
    private void addPlayer(Player p, String portalHealth) {
        ...
        p.getInterfaceState().openOverlay(new Component(24525));
        ...
        p.getPulseManager().run(new Pulse() {

            @Override
            public boolean pulse() {
                if (p.getExtension(PestControlSession.class) == null) {
                    return true;
                }
                if (p.getDialogueInterpreter().getDialogue() != null) {
                    return false;
                }
                if (p.getInterfaceState().getOverlay() == null || p.getInterfaceState().getOverlay().getId() != 24525) {
                    p.getInterfaceState().openOverlay(new Component(24525));
                }
                return false;
            }
        });
        p.getActionSender().sendString("200", 24535);
        for (int i = 0; i < 4; i++) {
            p.getActionSender().sendString(portalHealth, 24538 + i);
        }
        ...
        p.getActionSender().sendString("<col=FF0000>0", 24536);
        for (int configId = 203; configId < 211; configId++) {
            p.getActionSender().sendInterfaceConfig(configId, 0);
        }
    }
    ...
    private void spawnNPCs() {
        ...
        addNPC(squire = NPC.create(3782 + (RandomUtil.RANDOM.nextBoolean() ? 3 : 0), l.transform(32, 32, 0)));
        for (NPC npc : portals) {
            addNPC(npc);
