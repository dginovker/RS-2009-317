...
public final class PCObjectHandler extends OptionHandler {
    ...
    private void startActivity(Player p, String name, Location destination) {
        if (ActivityManager.start(p, name, false)) {
            p.getActionSender().sendMessage("You board the lander.");
    ...
    public boolean handle(Player player, Node node, String option) {
        ...
        if (option.equals("cross")) {
            if (player.getFamiliarManager().hasFamiliar()) {
                player.getActionSender().sendMessage("You can't take a follower on the lander.");
                return true;
            }
            switch (object.getId()) {
                case 14315: //Novice
    ...
    private void repair(Player player, PestControlSession session, GameObject object, int newId) {
        if (!player.getInventory().contains(Item.HAMMER, 1)) {
            player.getActionSender().sendMessage("You'll need a hammer to make any repairs!");
            return;
        }
        if (!player.getInventory().remove(new Item(1511, 1))) {
            player.getActionSender().sendMessage("You'll need some logs to make any repairs!");
            return;
        }
