...
public final class PCSquireNPC extends AbstractNPC {
    ...
    public boolean isAttackable(Entity entity, CombatStyle style) {
        return !(DeathTask.isDead(this) || entity instanceof Player);
    }
    ...
    public void tick() {
        ...
        if (updateLifepoints && session != null && (session.getTicks() % 10 == 0)) {
            if (getSkills().getLifepoints() > 50) {
                session.sendString("<col=00F800>" + Integer.toString(getSkills().getLifepoints()), 24535);
            } else {
                session.sendString("<col=FF0000>" + Integer.toString(getSkills().getLifepoints()), 24535);
            }
