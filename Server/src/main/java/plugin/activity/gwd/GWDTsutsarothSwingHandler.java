...
public final class GWDTsutsarothSwingHandler extends CombatSwingHandler {
    ...
    public int swing(Entity entity, Entity victim, BattleState state) {
        ...
        if (RandomUtil.randomize(10) < 4) {
            ticks += (int) Math.ceil(entity.getLocation().getDistance(victim.getLocation()) * 0.3);
            style = CombatStyle.MAGIC;
        } else if (RandomUtil.randomize(10) == 0) {
            if (special = (victim instanceof Player)) {
                ((Player) victim).getActionSender().sendMessage("K'ril Tsutsaroth slams through your protection prayer, leaving you feeling drained.");
            }
    ...
    public void impact(Entity entity, Entity victim, BattleState battleState) {
        if (battleState.getStyle() == CombatStyle.MAGIC) {
