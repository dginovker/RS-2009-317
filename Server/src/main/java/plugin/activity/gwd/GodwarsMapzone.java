...
public final class GodwarsMapzone extends MapZone implements Plugin<Object> {
    ...
    public boolean enter(Entity e) {
        if (e instanceof Player) {
            ...
            int componentId = 21346;
            if (ZAMORAK_FORTRESS.insideBorder(player.getLocation().getX(), player.getLocation().getY())) {
                componentId = 21346; // TODO Darker
            }
            openOverlay(player, componentId);
            // TODO godwars killcount upon entry for members
            /*if (player.getDonorManager().isSuperDonor() || player.getDonorManager().isExtremeDonor()) {
                for (GodWarsFaction faction : GodWarsFaction.values()) {
                    if (getKillcount(player, faction) < (player.getDonorManager().isSuperDonor() ? 20 : 30)) {
                        increaseKillcount(player, faction, player.getDonorManager().isSuperDonor() ? 20 : 30);
                    }
                }
            }*/
        }
    ...
    private void openOverlay(Player player, int componentId) {
        ...
        player.getInterfaceState().openOverlay(new Component(componentId));
        int child = 21353;
        for (GodWarsFaction faction : GodWarsFaction.values()) {
            int amount = player.getAttribute("gwd:" + faction.name().toLowerCase() + "kc", 0);
            player.getActionSender().sendString(Integer.toString(amount), child + faction.ordinal());
        }
    }
    ...
    public void locationUpdate(Entity e, Location last) {
        if (e instanceof Player) {
            Player player = (Player) e;
            Component c = player.getInterfaceState().getOverlay();
            //  if ((c == null || c.getId() != 598) && inZamorakFortress) {
            //      openOverlay(player, 598);
            /*} else */
            if ((c == null)) {// || c.getId() == 598) && !inZamorakFortress) {
                openOverlay(player, 21346);
            }
        }
    }
    ...
    public boolean interact(Entity e, Node target, Option option) {
        if (target instanceof GameObject) {
            ...
            if (object.getId() == 26295 || object.getId() == 26296) {
                if (e.getSkills().getStaticLevel(Skills.AGILITY) < 70) {
                    ((Player) e).getActionSender().sendMessage("You need an Agility level of 70 to enter here.");
                    return true;
                }
    ...
    private void handleAltar(Player player, String option, GodWarsFaction faction, Location destination) {
        ...
        if (player.getAttribute("gwd:altar-recharge", 0L) > System.currentTimeMillis()) {
            player.getActionSender().sendMessage("The gods blessed you recently - this time they ignore your prayers.");
            return;
        }
        if (player.inCombat()) {
            player.getActionSender().sendMessage("You can't use the altar while in combat.");
            return;
        }
        ...
        player.saveAttribute("gwd:altar-recharge", System.currentTimeMillis() + 600_000);
    }
    ...
    private void handleRopeTie(Player player, int type) {
        if (player.getSkills().getStaticLevel(Skills.AGILITY) < 70) {
            player.getActionSender().sendMessage("You need an Agility level of 70 to enter here.");
            return;
        }
        if (!player.getInventory().remove(new Item(954))) {
            player.getActionSender().sendMessage("You don't have a rope to tie on this rock.");
            return;
        }
    ...
    private void handlePillarGrapple(final Player player, final GameObject object) {
        if (player.getSkills().getStaticLevel(Skills.RANGE) < 70) {
            player.getActionSender().sendMessage("You need a Range level of 70 to enter here.");
            return;
        }
        if (player.getEquipment().getNew(Equipment.SLOT_AMMO).getId() != 9419) {
            player.getActionSender().sendMessage("You need a mithril grapple to cross this.");
            return;
        }
        RangeWeapon weapon = RangeWeapon.get(player.getEquipment().getNew(3).getId());
        if (weapon == null || weapon.getType() != 1) {
            player.getActionSender().sendMessage("You need to wield a crossbow to fire a mithril grapple.");
            return;
        }
        player.lock(4);
    ...
    private void handleBigDoor(final Player player, final GameObject object, boolean checkLocation) {
        ...
        if (checkLocation && player.getLocation().getX() > object.getLocation().getX()) {
            World.submit(new MovementPulse(player, object.getLocation()) {
        ...
        if (player.getSkills().getStaticLevel(Skills.STRENGTH) < 70) {
            player.getActionSender().sendMessage("You need a Strength level of 70 to enter here.");
            return;
        }
        if (!player.getInventory().contains(Item.HAMMER, 1)) {
            player.getActionSender().sendMessage("You need a hammer to bang on the door.");
            return;
        }
        player.getActionSender().sendMessage("You bang on the big door.");
    ...
    private boolean handleChamberEntrance(Player player, GameObject object) {
        ...
        if (dir.getStepX() != 0) {
            if (player.getLocation().getX() == object.getLocation().transform(dir.getStepX(), 0, 0).getX()) {
                player.getActionSender().sendMessage("You can't leave through this door. The altar can teleport you out.");
                return true;
            }
        ...
        if (player.getAttribute("gwd:" + name + "kc", 0) < 40) {
            player.getActionSender().sendMessage("You need 40 " + TextUtils.formatDisplayName(name) + " kills to enter this.");
            return true;
        }
    ...
    private void handleIceBridge(final Player player, final GameObject object) {
        if (player.getSkills().getStaticLevel(Skills.HITPOINTS) < 70) {
            player.getActionSender().sendMessage("You need 70 Hitpoints to cross this bridge.");
            return;
        }
        player.lock(7);
        World.submit(new Pulse(1, player) {
                    ...
                    public boolean pulse() {
                        switch (counter++) {
                            ...
                            case 5:
                                ...
                                player.getActionSender().sendMessage("Dripping, you climb out of the water.");
    ...
    public void increaseKillcount(Player p, GodWarsFaction faction, int increase) {
        ...
        amount = Integer.max(0, Math.min(4000, amount));
        p.saveAttribute(key, amount);
        p.getActionSender().sendString(amount == 4000 ? "Lots!" : Integer.toString(amount), childId);
    }
     ...
     * @param faction  The god wars faction.
     */
    public int getKillcount(Player p, GodWarsFaction faction) {
        if (faction == null) {
            return 0;
        }
        return p.getAttribute("gwd:" + faction.name().toLowerCase() + "kc", 0);
    }
    ...
    static enum GodWarsFaction {
        ARMADYL(6222, 6246,
            87, 11694, 11718, 11720, 11722, 12670, 12671),
        BANDOS(6260, 6283,
            11061, 11696, 11724, 11726, 11728),
        SARADOMIN(6247, 6259,
            1718, 11874, 2412, 2415, 2661, 2663, 2665, 2667, 3479, 3675, 3489, 3840, 4682,
            6762, 8055, 10384, 10386, 10388, 10390, 10440, 10446, 10452, 10458, 10464, 10470,
            11181, 11698, 11730),
        ZAMORAK(6203, 6221,
            11716, 11700, 11876, 2414, 2417, 2653, 2655, 2657, 2659, 3478, 3674, 3841, 3842,
            3852, 4683, 6764, 8056, 10368, 10370, 10372, 10374, 10444, 10450, 10456,
            10460, 10468, 10474, 10776, 10786, 10790, Item.ZAMORAKIAN_HASTA);
