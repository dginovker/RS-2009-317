...
public final class GWDKreeArraSwingHandler extends CombatSwingHandler {
    ...
    public int swing(Entity entity, Entity victim, BattleState state) {
        ...
        if (victim.getLocation().withinDistance(entity.getCenterLocation(), distance) && RandomUtil.RANDOM.nextBoolean()) {
            int hit = 0;
            ...
            for (BattleState s : targets) {
                CombatStyle style = RandomUtil.randomize(10) < 3 ? CombatStyle.MAGIC : CombatStyle.RANGE;
                s.setStyle(style);
                int hit = 0;
    ...
    public void impact(Entity entity, Entity victim, BattleState battleState) {
        ...
        for (BattleState bs : battleState.getTargets()) {
            ...
            bs.getVictim().getImpactHandler().handleImpact(entity, hit, bs.getStyle(), bs);
        }
    }
