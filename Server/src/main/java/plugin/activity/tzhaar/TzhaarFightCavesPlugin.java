...
public final class TzhaarFightCavesPlugin extends ActivityPlugin {
    ...
    private static final Location[] SPAWN_LOCATIONS = {
        Location.create(8, 44, 0),
        Location.create(38, 47, 0),
        Location.create(5, 9, 0),
        Location.create(46, 21, 0),
        Location.create(28, 28, 0),
    };
    ...
    public boolean canLogout(Player p) {
        if (!p.getAttribute("fc_safe_logout", false)) {
            ...
            p.getActionSender().sendMessage("<col=4C0E00>Your logout request has been received. The minigame will be paused after this wave.");
            p.getActionSender().sendMessage("<col=4C0E00>Clicking logout BEFORE finishing this wave will log you out without saving your");
            p.getActionSender().sendMessage("<col=4C0E00>progress.");
            p.getActionSender().sendMessage("<col=4C0E00>Clicking logout AFTER finishing this wave will log you out and save your progress.");
            return false;
        }
    ...
    public boolean start(final Player player, boolean login, Object... args) {
        ...
        if (!login) {
            int wave = 0;
            if (player.getSavedData().getActivityData().isCompletedFightCaves()) {
                wave = player.getDonorManager().getDonorStatus().getWaveId();
            }
            ...
            pulse = new Pulse(1, player) {
                ...
                public boolean pulse() {
                    if (count++ == 0) {
                        ...
                        player.getDialogueInterpreter().sendDialogues(2617, 9844, "You're on your own now, JalYt.", "Prepare to fight for your life!");
                    } else if (count == 16) {
                        spawnWave(finalWave);
                        if (finalWave != 0) {
                            player.getActionSender().sendMessage("You have automatically been advanced to wave " + (finalWave + 1) + " because you are a " + player.getDonorManager().getDonorStatus().toString().toLowerCase().replace("_", " ") + ".");
                        }
                        player.getWalkingQueue().reset();
                        return true;
                    }
            ...
            if (player.getAttribute("fc_wave", 0) == 62) {
                player.getDialogueInterpreter().sendDialogues(2617, 9844, "Look out, here comes TzTok-Jad!");
            }
    ...
    public void leave(Player player, int wave) {
        ...
        if (wave == 63) {
            ...
            player.getDialogueInterpreter().sendDialogues(2617, 9844, "You even defeated TzTok-Jad, I am most impressed!", "Please accept this gift as a reward.");
            player.getSavedData().getActivityData().setCompletedFightCaves(true);
        } else if (wave <= 1) {
            player.getDialogueInterpreter().sendDialogues(2617, 9844, "Well I suppose you tried... better luck next time.");
        } else {
            player.getDialogueInterpreter().sendDialogues(2617, 9844, "Well done in the cave, here, take this TokKul as a reward.");
        }
        int amount = (int) ((wave << 7) * MinigameSpotlight.Companion.getExpMultiplier(this.getName()));
    ...
    public boolean leave(final Entity e, boolean logout) {
        ...
        if (logout) {
            ...
            player.saveAttribute("fc_offset", (l.getX() - getBase().getX()) << 8 | (l.getY() - getBase().getY()));
        } else {
            player.removeAttribute("fc_offset");
    ...
    public void spawnWave(int wave) {
        ...
        player.getActionSender().sendMessage("Current wave: " + (wave + 1) + ".");
    }
    ...
    public boolean interact(Entity e, Node target, Option option) {
        if (target instanceof GameObject && target.getId() == 11834) {
            leave((Player) e, e.getAttribute("fc_wave", 0));
            player.removeAttribute("fc_wave");
            player.removeAttribute("fc_offset");
            return true;
