...
public final class TzhaarFightCaveNPC extends AbstractNPC {
    ...
    static class CombatAction extends CombatSwingHandler {
        ...
        public int swing(Entity entity, Entity victim, BattleState state) {
            ...
            int pulses = 1;
            if (jad) {
                main = CombatStyle.values()[1 + RandomUtil.RANDOM.nextInt(2)];
            }
            if (main != CombatStyle.MELEE) {
                ...
                if (CombatStyle.MELEE.getSwingHandler().canSwing(entity, victim) != InteractionType.NO_INTERACT && RandomUtil.random(10) < 4) {
                    ...
                    pulses += (int) Math.ceil(entity.getLocation().getDistance(victim.getLocation()) * 0.4);
                }
            }
            int max = calculateHit(entity, victim, 1.0);
            int hit = 0;
            boolean heal = victim instanceof NPC || ((npc.getId() == 2741 || npc.getId() == 2742) && RandomUtil.RANDOM.nextInt(4) < 1);
            if (((NPC) entity).getId() == 2746) {
                NPC j = (NPC) entity.getAttribute("fc_jad", entity);
                if (entity.getLocation().withinDistance(j.getCenterLocation(), (j.size() >> 1) + 1) && RandomUtil.random(4) < 2) {
                    heal = true;
                    state.setVictim(j);
                    victim = j;
                }
            ...
            if (heal || isAccurateImpact(entity, victim)) {
                ...
                hit = RandomUtil.random(max) + (heal ? 5 : 0);
            }
