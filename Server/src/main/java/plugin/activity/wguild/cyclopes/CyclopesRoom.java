...
public final class CyclopesRoom extends MapZone implements Plugin<Object> {
        ...
        public boolean pulse() {
            ...
            for (Iterator<Player> it = PLAYERS.iterator(); it.hasNext(); ) {
                ...
                if (current % 100 == 0) {
                    ...
                    p.getActionSender().sendMessage("10 of your tokens crumble away.");
                }
    ...
    public boolean death(Entity e, Entity killer) {
        if (killer instanceof Player && e instanceof NPC && (e.getId() == 4292 || e.getId() == 4291)) {
            ...
            int percentage = /*player.getDonorManager().isDonor() ? 20 :
                    player.getDonorManager().isSuperDonor() ? 30 :
                            player.getDonorManager().isExtremeDonor() ? 45 :*/ 0;
            // TODO upgrade for members?
            chance -= player.getDonorManager().getAdditional(chance, percentage);
            if (RandomUtil.randomize(chance) == 0) {
    ...
    private static int getDefenderIndex(Player player) {
        ...
        for (int i = DEFENDERS.length - 1; i >= 0; i--) {
            int id = DEFENDERS[i];
            if (player.getEquipment().getNew(Equipment.SLOT_SHIELD).getId() == id || player.getInventory().contains(id, 1)) {
                index = i;
                break;
            }
        }
