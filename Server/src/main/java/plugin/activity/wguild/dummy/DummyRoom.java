...
public final class DummyRoom extends OptionHandler {
    ...
    private static enum Dummy {
        STAB(new GameObject(15629, 2857, 3549, 0, 10, 2), -1, WeaponInterface.BONUS_STAB),
        SLASH(new GameObject(15625, 2858, 3554, 0), -1, WeaponInterface.BONUS_SLASH),
        CRUSH(new GameObject(15628, 2859, 3549, 0, 10, 2), -1, WeaponInterface.BONUS_CRUSH),
        CONTROLLED(new GameObject(15627, 2855, 3552, 0, 10, 3), WeaponInterface.STYLE_CONTROLLED, -1),
        DEFENCE(new GameObject(15630, 2855, 3550, 0, 10, 3), WeaponInterface.STYLE_DEFENSIVE, -1),
        AGGRESSIVE(new GameObject(15626, 2860, 3553, 0, 10, 1), WeaponInterface.STYLE_AGGRESSIVE, -1),
        ACCURATE(new GameObject(15624, 2856, 3554, 0), WeaponInterface.STYLE_ACCURATE, -1),;
        ...
        World.submit(new Pulse(10) {
            ...
            public boolean pulse() {
                if (!activeDummy) {
                    ...
                    dummy = RandomUtil.getRandomElement(Dummy.values());
    ...
    public boolean handle(Player player, Node node, String option) {
        ...
        if (object.getId() == 15656) {
            player.getInterfaceState().open(new Component(412));
            return true;
        }
        if (object.getId() == dummy.getObject().getId()) {
            if (timeStamp == player.getAttribute("dummy_ts", -1)) {
                player.getActionSender().sendMessage("You have already hit a dummy this turn.");
                return true;
            }
            if (player.getProperties().getAttackStyle().getStyle() != dummy.getAttackStyle() && player.getProperties().getAttackStyle().getBonusType() != dummy.getBonusType()) {
                ...
                player.getActionSender().sendMessage("You whack the dummy with the wrong attack style.");
            } else {
                player.getSkills().addExperience(Skills.ATTACK, 15.0);
                player.animate(player.getProperties().getAttackAnimation());
                player.getActionSender().sendMessage("You whack the dummy successfully!");
