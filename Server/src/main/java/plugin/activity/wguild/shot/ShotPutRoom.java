...
public final class ShotPutRoom extends DialoguePlugin {
        ...
        PluginManager.definePlugin(new OptionHandler() {
            ...
            public boolean handle(final Player player, Node node, String option) {
                ...
                if (node instanceof GroundItem) {
                    player.getDialogueInterpreter().sendDialogues(lowWeight ? 4299 : 4300, FacialExpression.ANGRY, "Hey! You can't take that, it's guild property. Take one", "from the pile.");
                    return true;
                }
                if (player.getEquipment().get(Equipment.SLOT_WEAPON) != null
                    ...
                    player.getDialogueInterpreter().sendPlaneMessage("To throw the shot you need your hands free!");
                    return true;
                }
                if (player.getSettings().getRunEnergy() < 10) {
                    player.getDialogueInterpreter().sendPlaneMessage("You're too exhausted to throw the shot at this time. Take a break.");
                    return true;
                }
                player.lock(4);
    ...
    private static void throwShotPut(final Player player, int delay, final boolean lowWeight, final String message) {
        ...
        int distance = 1;
        if (RandomUtil.randomize(25 - cost) != 0) {
            int mod = (int) (RandomUtil.randomize(cost) * (1 - (player.getSettings().getRunEnergy() / (100 + cost))));
            distance += RandomUtil.randomize(12 - mod);
        }
        final boolean failed = distance < 2;
        final int tiles = distance;
        player.getActionSender().sendMessage("You take a deep breath and prepare yourself.");
        World.submit(new Pulse(delay, player) {
            ...
            public boolean pulse() {
                if (!thrown) {
                    ...
                    if (failed) {
                        player.getActionSender().sendMessage("You fumble and drop the shot on your toe. Ow!");
