...
public final class CatapultRoom extends MapZone implements Plugin<Object> {
    ...
    private static Pulse pulse = new Pulse(10) {
        ...
        public boolean pulse() {
            attack = RandomUtil.getRandomElement(CatapultAttack.values());
            World.submit(new Pulse(7) {

                @Override
                public boolean pulse() {
                    for (Player p : players) {
                        if (p.isActive() && p.getLocation().equals(TARGET)) {
                            if (p.getEquipment().getNew(Equipment.SLOT_SHIELD).getId() != SHIELD_ID) {
                            ...
                            if (p.getAttribute("catapult_def", CatapultAttack.MAGIC_MISSILE) != attack) {
                                p.getActionSender().sendMessage("You fail defending against the " + attack.name + ".");
                                p.getImpactHandler().manualHit(p, 3, HitsplatType.NORMAL);
                                p.animate(attack.fail);
                            } else {
                                p.getSkills().addExperience(Skills.DEFENCE, 10.0);
                                p.getActionSender().sendMessage("You successfully defend against the " + attack.name + ".");
            ...
            Audio audio = new Audio(1911, 10, 0);
            for (Player p : players) {
            ...
            public boolean handle(Player player, Node node, String option) {
                if (node instanceof Item) {
                    if (!player.getLocation().equals(TARGET)) {
                        player.getActionSender().sendMessage("You may not equip this shield outside the target area in the Warrior's Guild.");
                        return true;
                    }
                    if (player.getEquipment().get(Equipment.SLOT_WEAPON) != null) {
                        player.getDialogueInterpreter().sendPlaneMessage("You will need to make sure your sword hand is free to equip this", "shield.");
                        return true;
                    }
                    ItemDefinition.getOptionHandlers().get("wield").handle(player, node, option);
                    if (player.getEquipment().getNew(Equipment.SLOT_SHIELD).getId() == SHIELD_ID) {
                        player.getInterfaceState().removeTabs(2, 3, 5, 6, 7, 11, 12);
                        player.getInterfaceState().openTab(4, new Component(411));
                        player.getInterfaceState().setViewedTab(4);
                    }
                    return true;
                }
                player.getInterfaceState().open(new Component(410));
                return true;
            }
        });
    ...
    private static void unequipShield(Player player) {
        if (player.getEquipment().getNew(Equipment.SLOT_SHIELD).getId() == SHIELD_ID) {
            ...
            player.getEquipment().replace(null, Equipment.SLOT_SHIELD);
            player.getInterfaceState().restoreTabs();
            player.getInterfaceState().openTab(4, new Component(387));
        }
    }
}
