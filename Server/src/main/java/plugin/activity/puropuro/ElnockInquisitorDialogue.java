...
public final class ElnockInquisitorDialogue extends DialoguePlugin {
        ...
        switch (stage) {
            ...
            case 30:
                if (!player.getSavedData().getActivityData().isElnockSupplies()) {
                    ...
                    npc("I have already given you some equipment.");
                }
                stage++;
                break;
            case 31:
    ...
    public static void openShop(Player player) {
        ...
        player.getInterfaceState().open(new Component(540));
        for (int i = 0; i < ElnockExchange.values().length; i++) {
            ElnockExchange e = ElnockExchange.values()[i];
            player.getActionSender().sendItemZoomOnInterface(e.getSendItem(), 115, 540, vals[i]);
        }
    }
    ...
    public static final class ElnockExchangeInterfaceHandler extends ComponentPlugin {
        ...
        public boolean handle(Player player, Component component, int opcode, int button, int slot, int itemId) {
            ...
            if (button == 126) {
                ...
                if (exchange == null) {
                    player.getActionSender().sendMessage("Making a selection before confirming.");
                    return true;
                }
                if (!exchange.hasItems(player)) {
                    player.getActionSender().sendMessage("You don't have the required implings in a jar to trade for this.");
                    return true;
                }
                if (exchange == ElnockExchange.JAR_GENERATOR && player.getInventory().contains(ElnockExchange.JAR_GENERATOR.getReward())) {
                    player.getActionSender().sendMessage("You can't have more than one jar generator at a time.");
                    return true;
                }
                if (!player.getInventory().hasRoomFor(exchange.getReward())) {
        ...
        public enum ElnockExchange {
            IMP_REPELLANT(114, 444928, 11271, new Item(11262), new Item(11238, 3), new Item(11240, 2), new Item(11242)),
            MAGIC_BUTTERFLY(117, 707072, 11268, new Item(11259), new Item(11242, 3), new Item(11244, 2), new Item(11246)),
            JAR_GENERATOR(120, 969216, 11267, new Item(11258), new Item(11246, 3), new Item(11248, 2), new Item(11250)),
