...
public final class PuroPuroPlugin extends MapZone implements Plugin<Object> {
    ...
    private void pushThrough(final Player player, final GameObject object) {
        if (player.getSkills().getStaticLevel(Skills.HUNTER) < 17) {
            player.getActionSender().sendMessage("You need a Hunting level of at least 17 to enter the maze.");
            return;
        }
        ...
        if (RegionManager.getObject(dest) != null) {
            player.getActionSender().sendMessage("An object on the other side is in your way.");
            return;
        }
        if (RandomUtil.random(2) == 0) {
            player.getActionSender().sendMessage("You use your strength to push through the wheat.");
        } else {
            player.getActionSender().sendMessage("You use your strength to push through the wheat. It's hard work though.");
        }
    ...
    public class PuroOptionHandler extends OptionHandler {
            ...
            switch (node.getId()) {
                ...
                case 11273:
                    if (!player.getZoneMonitor().isInZone("puro puro")) {
                        player.getActionSender().sendMessage("You can only use this in the Puro Puro Maze.");
                        break;
                    } else {
                        ...
                        player.getInterfaceState().openOverlay(new Component(541));
                    }
                    return true;
            }
            return true;
        ...
        private void handleJarGenerator(Player player, Item item, String option) {
            switch (option) {
                ...
                case "impling-jar":
                    ...
                    break;
                case "check":
                    player.getActionSender().sendMessage("Your jar generator has a charge percentage of " + getPercent(item) + ".");
                    break;
            }
        }
        ...
        private void generate(Player player, Item item, String option) {
            ...
            if (!hasPercent(item, percent)) {
                player.getActionSender().sendMessage("Your jar generator doesn't have enough charges to make another " + jar.getName().toLowerCase() + ".");
                return;
            }
            player.lock(5);
            ...
            player.getActionSender().sendMessage("Your jar generator generates a " + jar.getName().toLowerCase() + ".");
            if (getPercent(item) <= 0) {
                player.getInventory().remove(item);
                player.getActionSender().sendMessage("Your jar generator runs out of charges.");
            }
        }
    ...
    public static class WheatSet {
        ...
        public void setNextWhilt() {
            this.nextWhilt = World.getTicks() + RandomUtil.random(40, 300);
        }
