...
final class Filter {
	...
	public void decode(RSStream stream, Envelope soundEnveleope) {
		int count = stream.getByte();
		pairs[0] = count >> 4;
		pairs[1] = count & 0xf;
		if (count != 0) {
			unity[0] = stream.getShort();
			unity[1] = stream.getShort();
			int migration = stream.getByte();
			for (int k = 0; k < 2; k++) {
				for (int l = 0; l < pairs[k]; l++) {
					phases[k][0][l] = stream.getShort();
					magnitudes[k][0][l] = stream.getShort();
				}
