...
final class Synthesizer {
	...
	public void decode(RSStream stream) {
		...
		for (int index = 0; index < 10; index++) {
			...
			anIntArray107[index] = stream.getSmartA();
			anIntArray108[index] = stream.getSmartB();
		}

		delayTime = stream.getSmartB();
		delayDecay = stream.getSmartB();
		duration = stream.getShort();
