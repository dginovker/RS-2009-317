...
public final class Track {
    ...
    public static void unpack(RSStream stream) {
        output = new byte[0x6baa8];
        riff = new RSStream(output);
        Synthesizer.init();
        do {
            int id = stream.getShort();
            if (id == 65535) {
                return;
            }
            tracks[id] = new Track();
    ...
    public static RSStream data(int loops, int id) {
        if (tracks[id] != null) {
    ...
    private void decode(RSStream stream) {
        for (int synthesizer = 0; synthesizer < 10; synthesizer++) {
            int valid = stream.getByte();
            if (valid != 0) {
        ...
        loopStart = stream.getShort();
        loopEnd = stream.getShort();
    }
    ...
    private RSStream pack(int loops) {
        int size = mix(loops);
        riff.currentPosition = 0;
        riff.putInt(0x52494646);
        riff.writeLEInt(36 + size);
        riff.putInt(0x57415645);
        riff.putInt(0x666d7420);
        riff.writeLEInt(16);
        riff.putLEShort(1);
        riff.putLEShort(1);
