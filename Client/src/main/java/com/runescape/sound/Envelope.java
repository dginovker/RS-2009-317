...
final class Envelope {
	...
	public void decode(RSStream stream) {
		form = stream.getByte();
		start = stream.getInt();
		end = stream.getInt();
	...
	/**
	 * Decodes the segment data from the specified {@link com.runescape.net.RSStream}.
	 * 
	 * @param buffer
	 *            The buffer.
	 */
	public void decodeSegments(RSStream stream) {
		segments = stream.getByte();
		durations = new int[segments];
		peaks = new int[segments];
		for (int i = 0; i < segments; i++) {
			durations[i] = stream.getShort();
			peaks[i] = stream.getShort();
		}
