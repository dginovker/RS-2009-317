...
public final class MessageCensor {
    ...
    public static void load(CacheArchive archive) {
        RSStream fragments = new RSStream(
                archive.getEntry("fragmentsenc.txt"));
        RSStream bad = new RSStream(archive.getEntry("badenc.txt"));
        RSStream domain = new RSStream(archive.getEntry("domainenc.txt"));
        RSStream tldlist = new RSStream(archive.getEntry("tldlist.txt"));
        readValues(fragments, bad, domain, tldlist);
    }

    private static void readValues(RSStream fragments, RSStream badenc,
                                   RSStream domainenc, RSStream tldlist) {
        readBadEnc(badenc);
    ...
    private static void readTldList(RSStream stream) {
        int i = stream.getInt();
        tldList = new char[i][];
        tlds = new int[i];
        for (int j = 0; j < i; j++) {
            tlds[j] = stream.getByte();
            char ac[] = new char[stream.getByte()];
            for (int k = 0; k < ac.length; k++)
                ac[k] = (char) stream.getByte();
    ...
    private static void readBadEnc(RSStream stream) {
        int j = stream.getInt();
        badWords = new char[j][];
    ...
    private static void readDomainEnc(RSStream stream) {
        int i = stream.getInt();
        domains = new char[i][];
        initDomains(domains, stream);
    }

    private static void readFragmentsEnc(RSStream stream) {
        fragments = new int[stream.getInt()];
        for (int i = 0; i < fragments.length; i++)
            fragments[i] = stream.getShort();
    }

    private static void initBad(RSStream stream, char[][] words,
                                byte[][][] complexBadEnc) {
        for (int index = 0; index < words.length; index++) {
            char[] word = new char[stream.getByte()];
            for (int character = 0; character < word.length; character++)
                word[character] = (char) stream.getByte();

            words[index] = word;
            byte complex[][] = new byte[stream.getByte()][2];
            for (int l = 0; l < complex.length; l++) {
                complex[l][0] = (byte) stream.getByte();
                complex[l][1] = (byte) stream.getByte();
            }
    ...
    private static void initDomains(char[][] domains, RSStream stream) {
        for (int index = 0; index < domains.length; index++) {
            char[] domain = new char[stream.getByte()];
            for (int k = 0; k < domain.length; k++)
                domain[k] = (char) stream.getByte();
